...
const short = (value) => typeof value === "string" ? value.slice(0, 320) : undefined;
/** Forward bounded decisions and verified evidence references, never whole reports or command contexts. */
export function compactFlowEvent(event) {
    ...
    if (summary) {
        ...
        for (const key of ["reviewed_scopes", "carried_scopes", "executed_checks", "reused_checks"]) {
            ...
            if (Array.isArray(values))
                detail[key] = values.filter((value) => typeof value === "string").slice(0, 8).map(value => value.slice(0, 160));
        }
        if (Array.isArray(summary.reopened_scopes))
            detail.reopened_scopes = summary.reopened_scopes.slice(0, 8).flatMap(value => {
