...
export function permissionOwnerMatches(agent, request) {
    const handle = agent.backend_handle;
    if (agent.backend !== "codex-cli")
        return handle?.thread_id === request.thread_id;
...
export class PermissionRequests {
    ...
    create(connectionId, rpcId, request) {
        const record = { ...request, request_id: randomUUID(), state: "pending", decision: null, created_at: new Date().toISOString() };
    ...
    list(agentIds) {
        this.expire();
        return this.db.prepare("select * from permission_requests where agent_id in (select value from json_each(?)) order by rowid").all(JSON.stringify(agentIds))
            .map(row => { const { connection_id, rpc_id, response_json, lease_until, ...record } = this.decode(row); return record; });
    }
    decide(id, agentId, decision) {
        if (!["approve", "reject"].includes(decision))
            throw new ControllerError("Invalid permission decision.", "tool_error");
        const record = this.get(id);
        if (!record || record.agent_id !== agentId)
            throw new ControllerError("Permission request not found for this agent.", "auth_required");
        if (record.state !== "pending") {
            if (record.decision === decision && ["submitting", "sent", "resolved"].includes(record.state))
                return this.public(record);
            throw new ControllerError("This permission request is no longer pending. Refresh its current state.", "tool_error");
        }
        if (!record.choices.includes(decision) || (decision === "approve" && !record.scope_complete))
            throw new ControllerError("This decision is not supported for the visible request scope.", "unsupported_operation");
        ...
        if (!updated.changes && (current.decision !== decision || !["submitting", "sent", "resolved"].includes(current.state)))
            throw new ControllerError("A different decision already owns this request.", "tool_error");
    ...
    takeDecisions(connectionId) {
        return this.dispatchDecisions(connectionId, () => { });
    }
    dispatchDecisions(connectionId, send) {
        return this.db.transaction(() => {
            this.expire();
            const rows = this.db.prepare("select * from permission_requests where connection_id=? and state in ('pending','submitting','sent')").all(connectionId);
            const result = [];
            for (const row of rows) {
                const request = this.decode(row);
                const owner = this.db.prepare("select a.*, r.status as run_status from agents a join runs r on a.run_id=r.run_id where a.agent_id=?").get(request.agent_id);
                ...
                if (active && owner.status === "starting" && !handle)
                    continue;
                if (!active || !permissionOwnerMatches({ backend: owner.backend, backend_handle: handle }, request)) {
                    this.db.prepare("update permission_requests set state='unavailable' where request_id=?").run(request.request_id);
                    continue;
                }
    ...
    resolve(connectionId, rpcId, threadId) {
