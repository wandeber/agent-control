import { codexRunUsageWithEvidence } from "../adapters/codex-run-usage.js";
import { RunWakePolicy } from "./run-wake-policy.js";
const passive = (role) => role === "observer" || role === "orchestrator";
export function hasRunConversationUsage(agent) {
...
export class RunConversationUsage {
    ...
    window(runId) {
        ...
        if (!completion && run.status !== "stopped")
            return { start: run.created_at, end: null };
      ...
      and json_extract(payload_json,'$.action') in ('run_shutdown','run_shutdown_completed')`).all(runId);
    ...
    read(agent, window) {
        ...
        const row = this.store.db.prepare("select * from run_conversation_usage where run_id=? and agent_id=?")
            .get(agent.run_id, agent.agent_id);
        const cached = row?.thread_id === thread && row.started_at === window.start && row.ended_at === window.end ? JSON.parse(row.snapshot_json) : null;
        const baseline = agent.backend_handle?.usage_baseline;
        let observed;
        try {
            observed = codexRunUsageWithEvidence(thread, window.start, window.end, baseline?.rollout_path);
        }
        catch {
            return cached;
        }
        ...
        if (row?.thread_id === thread && row.started_at === window.start) {
            const prior = JSON.parse(row.evidence_json), keys = new Set(observed.evidence.map(record => record.key));
            ...
            if (prior.some(record => record.at <= until && !keys.has(record.key)))
                return cached;
        }
        ...
        if (json !== row?.snapshot_json || evidence !== row?.evidence_json)
            this.store.db.prepare(`insert into run_conversation_usage
