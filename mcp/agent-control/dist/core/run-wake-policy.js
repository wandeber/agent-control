...
export function sessionObservationKey(agent) {
...
export class RunWakePolicy {
    ...
    ) select run_id from tree`).all(runId).map(row => row.run_id);
    }
    completion(runId) {
        const ids = this.runIds(runId);
        let workers = 0, flows = 0, failed = false, cancelled = false;
        for (const id of ids) {
            if (this.store.db.prepare("select 1 from user_questions q join agents a on a.agent_id=q.agent_id join runs r on r.run_id=q.run_id where q.run_id=? and q.state='pending' and a.unregistered_at is null and a.status not in ('stopping','stopped') and r.status not in ('stopping','stopped')").get(id))
                return null;
            ...
            const unusedRoles = new Set();
            const resolvedFlowWorkers = new Set();
            for (const instance of instances) {
                if (instance.status !== "completed" && instance.status !== "cancelled")
                    return null;
                cancelled ||= instance.status === "cancelled";
                for (const owner of Object.values(this.runtime(instance.flow_instance_id)?.owners ?? {}))
                    unusedRoles.add(owner);
                if (instance.status === "completed") {
                    for (const owner of Object.values(this.runtime(instance.flow_instance_id)?.owners ?? {}))
                        resolvedFlowWorkers.add(owner);
                    for (const step of this.store.listFlowStepInstances(instance.flow_instance_id))
                        if (step.agent_id)
                            resolvedFlowWorkers.add(step.agent_id);
                }
            }
            flows += instances.length;
            if (this.store.listOrchestratorActions({ runId: id }).some(action => action.status === "pending" || action.status === "claimed"))
                return null;
            for (const agent of this.store.listAgents({ runId: id })) {
                if (!agent.unregistered_at && this.store.listGoals(agent.agent_id).some(goal => !["complete", "cancelled"].includes(goal.status)))
                    return null;
                // Attached external sessions are real work. Only conversation/coordinator identities are passive.
                if (agent.unregistered_at || agent.role === "observer" || agent.role === "orchestrator")
                    continue;
                const attempt = this.store.getLatestAgentStartAttempt(agent.agent_id);
                if (attempt && ["prepared", "invoking", "ambiguous"].includes(attempt.phase))
                    return null;
                if (agent.status === "planned" && !agent.backend_handle && agent.work_generation === 0 && unusedRoles.has(agent.agent_id))
                    continue;
                workers++;
            ...
            if (workers === priorWorkers && !instances.length && run.status !== "stopped" &&
                !ids.some(child => this.store.getRun(child)?.parent_run_id === id))
                return null;
        }
    ...
    wakes(event, observer, owners) {
        ...
        const operational = owners.length > 0 || sameThread(this.store.getRun(observer.run_id)?.created_by_agent_id) ||
            this.store.listFlowInstances({ runId: observer.run_id }).some(flow => this.store.listFlowStepInstances(flow.flow_instance_id).some(step => step.status === "active" && sameThread(step.agent_id)));
        ...
        if (event.type === "agent.status_changed" && payload.permission_state === "pending" && typeof payload.permission_request_id === "string") {
            ...
            if (request?.state !== "pending" || !agent || agent.unregistered_at || request.agent_id !== agent.agent_id || !permissionOwnerMatches(agent, request))
                return false;
            const requester = this.store.db.prepare("select thread_id from run_requesters where run_id=?").get(event.run_id ?? observer.run_id);
        ...
        if (event.type === "question.requested" || event.type === "question.answered") {
            const question = this.store.db.prepare("select state from user_questions where question_id=?").get(String(payload.question_id));
            if (!question || (event.type === "question.requested" && question.state !== "pending"))
                return false;
            const requester = this.store.db.prepare("select thread_id from run_requesters where run_id=?").get(event.run_id ?? observer.run_id);
        ...
        if (event.type === "flow.notification") {
            if (payload.reason === "coordinator_gate") {
                ...
                if (stepId && this.store.getFlowStepInstance(stepId)?.status !== "active")
                    return false;
            ...
            const action = payload.orchestrator_action;
            if (action)
                return Boolean(action.orchestrator_agent_id && owners.includes(action.orchestrator_agent_id));
            return operational && !INFORMATIONAL.has(String(payload.reason));
        }
        if (operational)
            return ROUTING.has(event.type) || needsInput;
        if (event.type === "flow.step_blocked")
            return true;
    ...
    initialSequence(runId, observer, owners, current) {
      ...
      where e.run_id in (select value from json_each(?)) and ((e.type = 'flow.notification' and s.status = 'active' and json_extract(e.payload_json, '$.reason') = 'coordinator_gate')
        ...
        or e.type = 'question.requested') order by o.sequence`).all(JSON.stringify(this.runIds(runId)));
        for (const row of rows) {
            if (String(row.run_id) !== runId && !this.store.db.prepare("select 1 from run_observers where run_id = ? and thread_id = ?")
                .get(String(row.run_id), String(observer.backend_handle?.thread_id ?? "")))
                continue;
            const event = { event_id: String(row.event_id), run_id: String(row.run_id), agent_id: row.agent_id ? String(row.agent_id) : null,
                type: String(row.type), created_at: String(row.created_at), payload: JSON.parse(String(row.payload_json)) };
            if (this.wakes(event, observer, owners))
                return Math.max(0, Number(row.sequence) - 1);
        }
        return current;
    }
}
