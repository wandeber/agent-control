...
import { closeSync, constants, fchmodSync, fstatSync, fsyncSync, lstatSync, mkdirSync, openSync, readFileSync, readdirSync, renameSync, unlinkSync, writeFileSync } from "node:fs";
...
export class LocalCredentialStore {
    ...
    persistBridgeCredential(credential) {
        const record = {
            version: 1,
            kind: "bridge",
    ...
    persistActionClaim(claim, action) {
        if (action.action_id !== claim.action_id ||
    ...
    findBridgeToken(input) {
        const records = this.findActiveBridgeCredentials(input);
        if (records.length === 0) {
            return null;
        }
        if (records.length > 1) {
            throw credentialStoreError("credential_store_invalid_record", "Multiple local bridge credentials match; pass --bridge-grant explicitly.");
        }
    ...
    cleanupStaleCredentials(limit = 100) {
        const diagnostics = [];
        let remaining = Math.max(0, limit);
        for (const kind of ["bridge", "action_claim"]) {
            if (remaining === 0) {
                break;
            }
            let entries;
    ...
    findUniqueBridgeCredential(input) {
     ...
     */
    findActiveBridgeCredentials(input) {
    ...
    hasActiveBridgeAuthority(record) {
    ...
    readBridgeCredential(bridgeGrantId) {
    ...
    writeCredential(kind, id, value) {
        ...
        try {
            descriptor = openNoFollow(temporary, constants.O_CREAT | constants.O_EXCL | constants.O_WRONLY, PRIVATE_FILE_MODE);
    ...
    assertBridgeExpectedBinding(record, input) {
        if (record.run_id !== input.runId ||
            record.orchestrator_agent_id !== input.orchestratorAgentId ||
            record.owner_task_path !== input.ownerTaskPath ||
            (input.ownerTaskIdentity !== undefined &&
    ...
    removeCredentialFile(kind, id) {
        const path = this.credentialPath(kind, id);
        safeLstatRegularFile(path);
        unlinkSync(path);
    }
}
function publicBridgeGrantRef(record) {
    return {
...
function publicActionClaimRef(record) {
    return {
...
function ensurePrivateDirectory(path, recursive, enforcePrivateMode = true) {
    let created = false;
    try {
...
function safeLstatRegularFile(path) {
...
function directChildPath(directory, filename) {
...
function bridgeGrantExpired(grant) {
    return Boolean(grant.expires_at && Date.parse(grant.expires_at) <= Date.now());
}
function cleanupDiagnostic(kind, id, error) {
    const code = error instanceof ControllerError && error.details.code === "credential_store_invalid_record"
        ? "credential_store_invalid_record"
        : "credential_cleanup_failed";
    return { kind, id, code };
}
function credentialStoreError(code, message, details = {}) {
