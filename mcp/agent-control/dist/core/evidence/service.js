...
import { existsSync, realpathSync, writeFileSync, readdirSync } from 'node:fs';
import { resolve, dirname, relative, isAbsolute } from 'node:path';
import { z } from 'zod';
import { evidenceContextSchema, evidenceRequestSchema, evidenceReceiptSchema, } from './schema.js';
import { HdtProvider, fingerprint, immutableJson, privateDirectory, readRegular, sha256 } from './provider.js';
import { executeCheck, executeCheckGraph, prepareCheck, verifyCurrentCommand } from './commands.js';
...
export class EvidenceService {
    ...
    receiptPath(context, id) {
        if (!/^[A-Za-z0-9][A-Za-z0-9._-]{0,159}$/.test(id))
            throw new Error('Invalid evidence receipt ID.');
        return resolve(this.namespace(context), 'receipts', `${id}.json`);
    }
    ensureArtifactInput(context, input) {
        ...
        if (!roots.some(root => { const rel = relative(root, real); return !rel.startsWith('..') && !isAbsolute(rel); }))
            throw new Error('Artifact input is outside the repository and run storage.');
        readRegular(path);
        return path;
    }
    checkpointBinding(context, checkpointId, plan, create = false) {
        ...
        if (create)
            immutableJson(path, binding);
        else if (fingerprint(JSON.parse(readRegular(path).toString())) !== fingerprint(binding))
            throw new Error('Checkpoint belongs to an obsolete acceptance or plan revision.');
    }
    makeReceipt(context, provider, kind, status, payload, options = {}) {
        const record = { schema_version: 1, receipt_id: options.id ?? `ev-${randomUUID()}`, kind, status,
            provider: 'hdt-review-checkpoint', provider_version: provider.manifest.source_version, provider_sha256: provider.manifest.sha256,
    ...
    reviewIndexPath(context, kind, checkpointId) {
        return resolve(this.namespace(context), 'review-index', `${kind}-${checkpointId}.json`);
    }
    findPredecessorReceipt(context, kind, checkpointId) {
        ...
        if (existsSync(indexPath)) {
            ...
            if (index.kind !== kind || index.checkpoint_id !== checkpointId)
                throw new Error('Review predecessor index identity is invalid.');
            const receipt = evidenceReceiptSchema.parse(JSON.parse(readRegular(this.receiptPath(context, index.receipt_id)).toString()));
            if (receipt.receipt_id !== index.receipt_id || receipt.kind !== kind || receipt.checkpoint_id !== checkpointId)
                throw new Error('Review predecessor index does not identify its expected receipt.');
            return receipt;
        }
        ...
        for (const file of readdirSync(directory)) {
            let candidate;
            try {
                candidate = JSON.parse(readRegular(resolve(directory, file)).toString());
            }
            catch {
                continue;
            }
            ...
            if (identity.kind !== kind || identity.checkpoint_id !== checkpointId)
                continue;
            if (predecessor)
                throw new Error('Review predecessor receipt is ambiguous; start an explicit fresh full-review lineage.');
            predecessor = evidenceReceiptSchema.parse(candidate);
            if (file !== `${predecessor.receipt_id}.json`)
                throw new Error('Review predecessor filename does not match its receipt identity.');
        }
        return predecessor;
    }
    async execute(rawContext, rawRequest) {
        ...
        if (request.operation === 'read_receipt')
            return this.readReceipt(context, request.receipt_id);
        const provider = this.provider(context);
        if (request.operation === 'snapshot_artifact') {
            const path = this.ensureArtifactInput(context, request.path);
            const bytes = readRegular(path);
            const hash = sha256(bytes);
            const blob = resolve(this.namespace(context), 'artifacts', hash);
            privateDirectory(dirname(blob));
            if (!existsSync(blob))
                writeFileSync(blob, bytes, { flag: 'wx', mode: 0o600 });
            if (sha256(readRegular(blob)) !== hash)
                throw new Error('Artifact snapshot integrity failure.');
        ...
        if (request.operation === 'prepare_plan' || request.operation === 'prepare_result') {
            ...
            if (request.previous) {
                ...
                if (previous.acceptance_revision !== context.acceptanceRevision || (!plan && previous.plan_revision !== context.planRevision))
                    throw new Error('Changed acceptance or approved plan requires a fresh full-review lineage.');
            }
            const args = ['--checkpoint-id', request.checkpoint_id, '--plan', request.plan_path];
            if (request.previous)
                args.push('--previous', request.previous);
            if (!plan && request.operation === 'prepare_result') {
                if (request.base)
                    args.push('--base', request.base);
                if (request.all_changes)
                    args.push('--all-changes');
                if (request.isolated_worktree)
                    args.push('--isolated-worktree');
            ...
            return this.makeReceipt(context, provider, plan ? 'plan_checkpoint' : 'result_checkpoint', 'prepared', payload, { checkpointId: request.checkpoint_id, snapshot: payload.snapshot_sha256 });
        }
        if (request.operation === 'review_scope' || request.operation === 'diff_plan' || request.operation === 'diff_result') {
            ...
            if (request.include_patches)
                args.push('--include-patches');
            if (request.operation === 'review_scope')
                request.surfaces?.forEach(id => args.push('--surface', id));
            if (request.operation === 'diff_plan')
                request.sections?.forEach(id => args.push('--section', id));
            ...
            const reopened = payload.required_review_section_ids ?? payload.required_review_surface_ids ?? [];
            return this.makeReceipt(context, provider, scope ? 'review_scope' : request.operation === 'diff_plan' ? 'plan_delta' : 'result_delta', 'prepared', payload, { checkpointId, summary: { carried_count: carried.length, carried_scopes: carried.map(label), reopened_count: reopened.length,
        ...
        if (request.operation === 'record_plan_review' || request.operation === 'record_review') {
            ...
            if (previousId) {
                ...
                if (previous) {
                    if (previous.actor_id !== context.actorId)
                        throw new Error('Incremental review requires the same authenticated review owner.');
                    ...
                    if (request.draft.coverage_ledger?.mode === 'incremental' || existsSync(priorProviderReview)) {
            ...
            for (const source of new Set(sources))
                this.verifyReviewSource(context, source, previousId, kind);
            ...
            if (request.operation === 'record_review' && request.gate === 'planner') {
                const validation = request.source_receipt_ids.map(id => this.verifyReceiptSync(context, id))
                    ...
                    && receipt.payload.mechanical_report.validation_mode === 'complete_gate');
                if (!validation)
                    throw new Error('Planner review requires source_receipt_ids containing a complete validation receipt for this exact result.');
            ...
            if (request.operation === 'record_review')
                args.push('--gate', request.gate);
            const payload = provider.invoke(context, plan ? 'record-plan-review' : 'record-review', args, { flag: '--report', value: semanticDraft });
            return this.makeReceipt(context, provider, kind, payload.verdict === 'approved' ? 'approved' : 'rejected', payload, { checkpointId: request.checkpoint_id, snapshot: payload.snapshot_sha256, sources,
        ...
        if (request.operation === 'project_plan') {
            ...
            const args = ['--checkpoint-id', request.checkpoint_id];
            request.sections.forEach(section => args.push('--section', section));
        ...
        if (request.operation === 'validation_run')
            return this.runValidation(context, provider, request);
        ...
        if (expert.snapshot_sha256 !== validation.snapshot_sha256)
            throw new Error('Expert and validation evidence identify different results.');
    ...
    verifyInternal(context, receiptId, expected, visiting) {
        if (visiting.has(receiptId))
            throw new Error('Evidence source cycle detected.');
        ...
        try {
            ...
            if (fingerprint(body) !== record_sha256 || receipt.receipt_id !== receiptId || receipt.run_id !== context.runId || receipt.flow_instance_id !== context.flowInstanceId
                || receipt.provider_sha256 !== provider.manifest.sha256)
                throw new Error('Evidence receipt identity or integrity failed.');
            if (receipt.acceptance_revision !== context.acceptanceRevision || receipt.plan_revision !== context.planRevision)
                throw new Error('Evidence receipt belongs to an obsolete acceptance or plan revision.');
            if (expected.kind && receipt.kind !== expected.kind)
                throw new Error('Evidence receipt kind does not satisfy this gate.');
            if (expected.checkpointId && receipt.checkpoint_id !== expected.checkpointId)
                throw new Error('Evidence receipt checkpoint does not satisfy this gate.');
            if (expected.actorId && receipt.actor_id !== expected.actorId)
                throw new Error('Evidence receipt author does not own this review.');
            if (expected.requireApproved && !['approved', 'passed', 'verified'].includes(receipt.status))
                throw new Error('Evidence receipt has not passed its gate.');
            ...
            else if (expected.validationMode)
                throw new Error('A validation mode requires a validation receipt.');
            ...
            if (receipt.kind === 'artifact') {
                if (sha256(readRegular(payload.blob_path)) !== receipt.snapshot_sha256)
                    throw new Error('Artifact snapshot is corrupt.');
                if (expected.requireCurrent && sha256(readRegular(this.ensureArtifactInput(context, payload.path))) !== receipt.snapshot_sha256)
                    throw new Error('Artifact changed after it was captured.');
            }
            ...
            if (receipt.checkpoint_id) {
                ...
                if (receipt.kind === 'plan_review' || receipt.kind === 'plan_projection')
                    args.push('--require-review');
                if (receipt.kind === 'expert_review' || receipt.kind === 'planner_review')
                    args.push('--require-review', receipt.kind === 'expert_review' ? 'expert' : 'planner');
                ...
                if (receipt.snapshot_sha256 && verified.snapshot_sha256 && verified.snapshot_sha256 !== receipt.snapshot_sha256)
                    throw new Error('Provider checkpoint identity changed.');
                if (expected.requireCurrent)
                    provider.invoke(context, plan ? 'matches-plan' : 'matches', ['--checkpoint-id', receipt.checkpoint_id]);
            }
            if (['plan_review', 'planner_review', 'expert_review'].includes(receipt.kind)) {
                ...
                if (envelope.report_sha256 !== receipt.payload.report_sha256 || envelope.snapshot_sha256 !== receipt.snapshot_sha256)
                    throw new Error('Review receipt no longer matches its exact provider record.');
                if (receipt.kind === 'planner_review') {
                    const validation = sources.find(source => source.kind === 'validation' && source.snapshot_sha256 === receipt.snapshot_sha256
                        && source.status === 'passed' && source.payload.mechanical_report.validation_mode === 'complete_gate'
                        && fingerprint(source.payload.mechanical_report) === envelope.report.mechanical_validation_report_sha256);
                    if (!validation)
                        throw new Error('Planner review does not bind a verified complete mechanical source for its exact result.');
                }
            }
            if (receipt.kind === 'validation') {
                this.verifyValidation(context, receipt, sources, provider);
                if (expected.requireCurrent)
                    for (const record of z.array(executionSchema).parse(payload.executions))
                        verifyCurrentCommand(context, record, receipt.snapshot_sha256);
            }
            if (receipt.kind === 'closure') {
                const expert = sources.find(source => source.kind === 'expert_review');
                const validation = sources.find(source => source.kind === 'validation');
                if (!expert || expert.status !== 'approved' || !validation || validation.status !== 'passed' || expert.snapshot_sha256 !== validation.snapshot_sha256)
                    throw new Error('Closure sources are missing, stale or not GREEN.');
                if (expected.requireCurrent)
                    for (const record of z.array(executionSchema).parse(validation.payload.executions))
                        verifyCurrentCommand(context, record, validation.snapshot_sha256);
                if (expected.requireCurrent)
                    provider.invoke(context, 'verify-closure', ['--expert-checkpoint-id', expert.checkpoint_id], { flag: '--mechanical-report', value: validation.payload.mechanical_report });
            }
            return receipt;
        }
        finally {
    ...
    assertPlanIdentity(context, planSha256) {
    ...
    verifyReviewSource(context, id, previousId, kind, visiting = new Set()) {
    ...
    verifyValidation(context, receipt, sources, provider) {
        const payload = receipt.payload;
        const records = z.array(executionSchema).parse(payload.executions);
        if (new Set(records.map(record => record.check_id)).size !== records.length)
            throw new Error('Validation contains duplicate check IDs.');
        for (const record of records) {
            if (record.snapshot_sha256 !== receipt.snapshot_sha256)
                throw new Error('Command execution belongs to another result.');
            const file = resolve(this.namespace(context), 'executions', `${record.execution_id}.json`);
            if (fingerprint(JSON.parse(readRegular(file).toString())) !== fingerprint(record))
                throw new Error('Native command execution record is missing or corrupt.');
            if (record.disposition === 'reused') {
                ...
                const original = origin?.payload.executions?.find((item) => item.execution_id === record.source_execution_id);
                if (!original || original.verdict !== 'passed' || original.exit_status !== 0 || original.volatile || !original.context_complete
                    || original.identity_sha256 !== record.identity_sha256 || original.input_sha256 !== record.input_sha256)
                    throw new Error('Reused command origin does not prove this execution identity.');
            }
            else if (record.source_receipt_id !== null || record.source_execution_id !== null)
                throw new Error('Executed command must not claim a reuse origin.');
        }
        if (receipt.status === 'passed' && records.some(record => record.verdict !== 'passed' || record.exit_status !== 0))
            throw new Error('Passed validation includes a failed command.');
        if (records.length) {
            const stored = provider.verifyMechanical(context, receipt.checkpoint_id, payload.provider_mechanical.evidence_id);
            if (stored.report_sha256 !== payload.provider_mechanical.report_sha256)
                throw new Error('Validation no longer binds its canonical mechanical batch.');
        }
        if (payload.mechanical_report.checks.length !== records.length)
            throw new Error('Validation coverage and executed command set differ.');
        for (const check of payload.mechanical_report.checks) {
            ...
            if (!record || check.command !== `sha256:${record.identity_sha256}` || check.exit_status !== record.exit_status || check.verdict !== record.verdict
                || check.disposition !== record.disposition)
                throw new Error('Mechanical report does not bind its native execution receipt.');
        }
    ...
    validateCoverage(provider, context, checkpointId, mechanical) {
    ...
    async runValidation(context, provider, request) {
        ...
        this.assertPlanIdentity(context, manifest.plan.sha256);
        const snapshot = manifest.snapshot_sha256;
        const sources = [];
        ...
        for (const id of request.source_receipt_ids) {
            try {
                const source = this.verifyReceiptSync(context, id, { kind: 'validation' });
                if (source.status === 'passed')
                    sources.push(source);
            }
            catch { /* Conservative cache miss. */ }
        }
        ...
        const candidateIds = sources.flatMap(source => {
            const evidence = source.payload.provider_mechanical;
            return evidence?.evidence_id ? [evidence.evidence_id] : [];
        });
        ...
        if (candidateIds.length && request.checks.length) {
            ...
            for (const item of resolution.resolutions)
                if (item.disposition === 'reused')
                    canonicalHits.set(item.check_id, item.source.evidence_id);
        }
        const executions = await executeCheckGraph(request.checks, async (check) => {
            const input = prepared.get(check.check_id);
            const origin = !input.identity.volatile && input.identity.context_complete ? sources.flatMap(source => source.payload.executions.map(record => ({ source, record }))).find(({ source, record }) => record.check_id === check.check_id && record.verdict === 'passed' && record.exit_status === 0 && !record.volatile && record.context_complete
                && (input.identity.mode === 'declared_inputs' || canonicalHits.get(check.check_id) === source.payload.provider_mechanical?.evidence_id)
                && record.identity_sha256 === input.identity.identity_sha256 && record.input_sha256 === input.identity.input_sha256) : undefined;
            const executionId = `exec-${randomUUID()}`;
            const execution = origin ? { ...origin.record, ...input.identity, execution_id: executionId,
            ...
            if (fingerprint(after.identity) !== fingerprint(input.identity))
                throw new Error('Validation inputs or command context changed during execution.');
        ...
        const canonicalMechanical = executions.length ? provider.invoke(context, 'record-mechanical-evidence', ['--checkpoint-id', request.checkpoint_id, '--evidence-id', receiptId], { flag: '--report', value: { schema_version: 1, checkpoint_id: request.checkpoint_id, task_result_snapshot_sha256: snapshot,
...
function label(value) { return value.replace(/[\u0000-\u001f]/g, ' ').slice(0, 120); }
function mechanicalDescriptor(identity) {
