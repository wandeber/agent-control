...
function treeIdentity(root, input, external = false) {
    const path = external ? realpathSync(resolve(root, input)) : repositoryPath(root, input);
    const visit = (entry) => {
        const stat = lstatSync(entry);
        if (stat.isSymbolicLink())
            throw new Error('Declared validation inputs cannot contain symlinks.');
        if (stat.isFile())
            return { mode: stat.mode & 0o777, size: stat.size, mtime_ms: stat.mtimeMs, ctime_ms: stat.ctimeMs,
                inode: stat.ino, device: stat.dev, sha256: sha256(readFileSync(entry)) };
        if (!stat.isDirectory())
            throw new Error('Declared validation input is not a regular file or directory.');
...
function executablePath(command, cwd, env) {
...
export function prepareCheck(context, check, snapshot, executionRoot) {
    const cwd = repositoryPath(context.repoPath, check.cwd);
    if (!lstatSync(cwd).isDirectory())
        throw new Error('Validation cwd must be a directory.');
    ...
    for (const name of check.environment_names)
...
export function verifyCurrentCommand(context, identity, snapshot) {
    const recipe = identity.verification;
    const env = { PATH: process.env.PATH ?? '', LANG: 'C.UTF-8', TZ: 'UTC' };
    for (const name of recipe.environment_names)
        if (process.env[name] !== undefined)
            env[name] = process.env[name];
    ...
    const input = identity.mode === 'declared_inputs'
        ...
        : { snapshot_sha256: snapshot, repo_path: realpathSync(context.repoPath) };
    if (fingerprint(invocation) !== identity.identity_sha256 || fingerprint(input) !== identity.input_sha256)
        throw new Error('Command input, toolchain or environment changed after validation.');
}
export function readOnlySandboxAvailable() {
...
export async function executeCheck(prepared, executionId, snapshot) {
    ...
    try {
        if (check.sandbox === 'read_only') {
            if (!readOnlySandboxAvailable())
                throw new Error('Read-only command sandbox is unavailable; explicitly choose workspace execution or a supported backend.');
        ...
        const result = await new Promise((resolveResult, reject) => {
            ...
            let outPreview = '';
            let errPreview = '';
            child.stdout.on('data', (data) => { out.update(data); outBytes += data.length; outPreview = (outPreview + data.toString()).slice(-32_768); });
            child.stderr.on('data', (data) => { err.update(data); errBytes += data.length; errPreview = (errPreview + data.toString()).slice(-32_768); });
...
function sanitizeOutput(value) {
    ...
    for (const [key, secret] of Object.entries(process.env))
        if (secret && secret.length >= 6 && /secret|token|password|credential|api.?key/i.test(key))
...
export async function executeCheckGraph(checks, run) {
    const byId = new Map(checks.map(check => [check.check_id, check]));
    if (byId.size !== checks.length)
        throw new Error('Validation check IDs must be unique.');
    for (const check of checks)
        if (check.depends_on.some(id => !byId.has(id) || id === check.check_id))
            throw new Error('Invalid validation check dependency.');
    const pending = new Set(byId.keys());
    const completed = new Map();
    while (pending.size) {
        const ready = checks.filter(check => pending.has(check.check_id) && check.depends_on.every(id => completed.has(id)));
        if (!ready.length)
            throw new Error('Validation check dependencies contain a cycle.');
        const batch = [];
        const resources = new Set();
        for (const check of ready) {
            if (batch.length && (!check.independent || !batch[0].independent || check.resources.some(resource => resources.has(resource))))
                continue;
            batch.push(check);
            check.resources.forEach(resource => resources.add(resource));
        ...
        results.forEach((result, index) => { if (result.status === 'fulfilled') {
            completed.set(batch[index].check_id, result.value);
            pending.delete(batch[index].check_id);
        } });
    }
