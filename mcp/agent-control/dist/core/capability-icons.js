...
export function capabilityIcon(candidates, roots) {
    for (const candidate of candidates) {
        if (typeof candidate !== "string" || !candidate || /^[a-z][a-z\d+.-]*:/i.test(candidate))
            continue;
        ...
        for (const path of new Set(paths)) {
            try {
                ...
                if (!roots.some(root => { try {
                    return inside(canonical, realpathSync(root));
                }
                catch {
                    return false;
                } }))
...
function imageMime(extension, bytes) {
    if (extension === ".svg" && /^(?:<\?xml[^>]*>\s*)?(?:<!--[\s\S]*?-->\s*)*<svg(?:\s|>)/.test(bytes.toString("utf8").trimStart()))
    ...
    if ([".jpg", ".jpeg"].includes(extension) && bytes[0] === 255 && bytes[1] === 216 && bytes[2] === 255)
        return "image/jpeg";
    if (extension === ".webp" && bytes.toString("ascii", 0, 4) === "RIFF" && bytes.toString("ascii", 8, 12) === "WEBP")
        return "image/webp";
    if (extension === ".gif" && /^GIF8[79]a/.test(bytes.toString("ascii", 0, 6)))
        return "image/gif";
    return undefined;
}
