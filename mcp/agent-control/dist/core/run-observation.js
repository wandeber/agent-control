...
import { RunWakePolicy, sessionObservationKey } from "./run-wake-policy.js";
...
export class RunObservation {
    ...
    requesterThread(runId, visited = new Set()) {
      ...
      select r.run_id, b.thread_id from runs r join run_operator_bindings b on b.run_id=r.parent_run_id where r.run_id=?`).run(runId);
        const operator = this.store.db.prepare("select thread_id from run_operator_bindings where run_id=?").get(runId);
    ...
    observe(input) {
        ...
        return this.store.immediateTransaction(() => {
          ...
          select run_id, thread_id from run_requesters where run_id=? and thread_id=?`).run(run.run_id, threadId);
            ...
            const start = previous?.start_sequence ?? this.wakePolicy.initialSequence(run.run_id, agent, this.observationOwners(agent, run.run_id), this.currentSequence());
    ...
    async wait(input) {
        ...
        if (wakeOn !== "control" && wakeOn !== "all")
            throw new ControllerError("Invalid run wait wake policy.", "tool_error");
        ...
        const empty = (timedOut, closed) => ({ run_id: input.runId, observer_agent_id: input.observerAgentId,
            events: [], cursor: requestedCursor, completion: null,
        ...
        while (true) {
            ...
            if (!this.eventTypes(observer.observer_agent_id).length)
                return empty(false, true);
            const runIds = this.wakePolicy.runIds(input.runId);
            const freshExternalSessions = runIds.every(id => this.store.listAgents({ runId: id }).every(candidate => candidate.unregistered_at || candidate.backend !== "codex-session" || refreshedSessions.get(candidate.agent_id) === sessionObservationKey(candidate)));
            ...
            for (const runId of this.wakePolicy.runIds(input.runId).filter(id => id !== input.runId)) {
                const attached = this.store.db.prepare("select observer_agent_id, start_sequence from run_observers where run_id = ? and thread_id = ?")
                    .get(runId, observer.thread_id);
                const participant = attached ? this.store.getAgent(attached.observer_agent_id) : null;
                if (participant && !participant.unregistered_at)
                    scopes.set(runId, { agent: participant, owners: this.observationOwners(participant, runId), start: attached.start_sequence });
            }
        ...
        order by o.sequence limit 100`).all(runId, Math.max(sequence, scope.start), scope.agent.agent_id, JSON.stringify(scope.owners), scope.owners.length ? 1 : 0))
                .sort((a, b) => Number(a.sequence) - Number(b.sequence)).slice(0, 100);
            const events = [];
            for (const row of rows) {
                sequence = Number(row.sequence);
                const scope = scopes.get(String(row.run_id));
                const event = { event_id: String(row.event_id), run_id: String(row.run_id),
        ...
        where e.run_id = ? and o.sequence > ? and ${MATCHING_OBSERVER_SUBSCRIPTION} limit 1`)
            ...
            void this.controller.refreshObservedRuns(runIds).then(ids => { for (const item of ids)
                refreshedSessions.set(item.agent_id, item.key); });
            input.signal?.throwIfAborted();
