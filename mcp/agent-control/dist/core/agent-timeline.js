...
import { readNativeTimeline } from "../adapters/codex-timeline.js";
const millis = (value) => Date.parse(value);
export function activityIntervals(boundaries, until) {
    const intervals = [];
    let start = null, key = "", partial = false;
    for (const boundary of [...boundaries].sort((a, b) => a.at - b.at)) {
        if (!Number.isFinite(boundary.at)) {
            partial = true;
            continue;
        }
        if (boundary.kind === "start" && boundary.at <= until) {
            if (start !== null) {
                if (key === boundary.key && start === boundary.at)
                    continue;
                intervals.push({ start, end: boundary.at });
                partial = true;
            }
            start = boundary.at;
            key = boundary.key;
            ...
            if (start !== null) {
                if (boundary.key !== key && boundary.key !== "turn" && key !== "turn") {
                    partial = true;
                    continue;
                }
            ...
            else if (boundary.at <= until)
                partial = true;
        }
    }
    if (start !== null)
        intervals.push({ start, end: until, open: true });
    return { intervals, partial };
}
export function waitingIntervals(boundaries, until) {
    const starts = new Map(), intervals = [];
    for (const boundary of [...boundaries].sort((a, b) => a.at - b.at)) {
        if (!Number.isFinite(boundary.at) || boundary.at > until)
            continue;
        if (boundary.kind === "wait" && !starts.has(boundary.key))
            starts.set(boundary.key, boundary.at);
        if (boundary.kind === "resume" && starts.has(boundary.key)) {
            intervals.push({ start: starts.get(boundary.key), end: boundary.at });
            starts.delete(boundary.key);
        }
    ...
    for (const start of starts.values())
        intervals.push({ start, end: until });
    return intervals;
}
export function subtractWaits(active, waits) {
    return waits.reduce((remaining, wait) => remaining.flatMap(span => {
        if (wait.end <= span.start || wait.start >= span.end)
            return [span];
        const parts = [];
        if (wait.start > span.start)
            parts.push({ start: span.start, end: wait.start });
        if (wait.end < span.end)
            parts.push({ start: wait.end, end: span.end, open: span.open });
        return parts;
    }), active);
}
function controlBoundaries(history) {
    const boundaries = [];
    let running = false, waiting = false;
    const questionWaits = new Map();
        ...
        if (row.type.startsWith("question.wait_")) {
            ...
            if (row.type === "question.wait_started") {
                ...
                if (typeof p.expires_at === "string")
                    boundaries.push({ at: millis(p.expires_at), kind: "resume", key });
            }
            else
                questionWaits.delete(key);
            continue;
        }
        if (row.type === "question.answered") {
            for (const [key, id] of questionWaits)
                if (id === p.question_id) {
                    boundaries.push({ at, kind: "resume", key });
                    questionWaits.delete(key);
                }
            continue;
        }
        const status = typeof p.status === "string" ? p.status : { "agent.started": "running", "agent.completed": "completed", "agent.stopped": "stopped", "agent.failed": "failed", "agent.blocked": "blocked", "agent.unregistered": "stopped" }[row.type];
        if (!status)
            continue;
...
function nativeThread(agent) {
    if (!agent.backend.startsWith("codex-") || agent.backend_handle?.remote_session)
        return;
    const handle = agent.backend_handle;
    if (typeof handle?.thread_id === "string")
        return handle.thread_id;
    if (agent.backend === "codex-subagent" && typeof handle?.native_agent_id === "string")
        return handle.native_agent_id;
    if (agent.backend === "codex-cli" && typeof handle?.dir === "string") {
        try {
            const state = JSON.parse(readFileSync(join(handle.dir, "state.json"), "utf8"));
            if (typeof state.thread_id === "string")
                return state.thread_id;
        }
        catch { /* No persisted native identity yet. */ }
    }
}
/** Full lifecycle history, independent of the dashboard's truncated event feed. */
export function projectAgentTimeline(store, agents, window, generatedAt) {
    ...
    order by created_at,rowid`).all(JSON.stringify(agents.map(agent => agent.agent_id)));
    const rows = agents.map((agent) => {
        const control = controlBoundaries(history.filter(row => row.agent_id === agent.agent_id));
        const thread = nativeThread(agent), baseline = agent.backend_handle?.usage_baseline;
