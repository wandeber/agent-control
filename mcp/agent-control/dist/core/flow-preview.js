...
function previewPrompt(source, roots) {
    const result = { ...source, text: typeof source.text === "string" ? source.text : null, error: null };
    ...
    try {
        ...
        if (statSync(path).size > 512 * 1024)
            throw new Error("Prompt is too large to preview (512 KiB limit).");
        result.text = readFileSync(path, "utf8");
    }
    catch (cause) {
        result.error = cause instanceof Error ? cause.message : "Prompt unavailable.";
    }
    return result;
}
