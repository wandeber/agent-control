...
function assertSupportedCodexSubagentOptions(value) {
    ...
    for (const [roleId, roleValue] of Object.entries(rolesValue)) {
        ...
        if ("resolved_agent" in role)
            throw new ControllerError("resolved_agent is internal and cannot be supplied in a public flow config.", "tool_error", { role: roleId });
        if ("agent_ref" in role) {
            if (["prompt", "prompt_ref", "prompt_path"].some(key => key in role))
                throw new ControllerError("agent_ref cannot be combined with role prompt, prompt_ref, or prompt_path.", "tool_error", { role: roleId });
            if (role.backend !== undefined && role.backend !== "codex-cli")
                throw new ControllerError("agent_ref requires the codex-cli backend.", "unsupported_operation", { role: roleId, backend: role.backend });
        }
        const backend = role.backend ?? (role.agent_ref ? "codex-cli" : undefined);
        if (role.model_provider && backend !== "codex-cli")
            throw new ControllerError("model_provider is only supported by codex-cli roles.", "unsupported_operation", { role: roleId });
...
export function validateFlowConfigReferences(config) {
    ...
    for (const [stepId, step] of Object.entries(config.steps)) {
        ...
        if (step.agent_id && roleConfig?.agent_ref)
            throw new ControllerError("A referenced flow role cannot use a pre-existing step agent_id.", "tool_error", { step_id: stepId, role: step.role });
