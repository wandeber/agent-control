...
function defaultCatalogBasePath() {
    const here = dirname(fileURLToPath(import.meta.url));
    return here;
}
function ancestorFlowCatalogCandidates(basePath) {
    const candidates = [];
...
function marketplaceCacheFlowCatalogCandidate(basePath) {
...
function marketplaceCheckoutFlowCatalogCandidate(basePath) {
    const parts = resolve(basePath).split(sep);
    const marker = marketplaceCacheMarker(parts);
    if (!marker) {
        return null;
...
function marketplaceCacheMarker(parts) {
...
function listCatalogFlows(catalog) {
    if (!catalog.exists) {
        return [];
    }
    return readdirSync(catalog.root_path, { withFileTypes: true })
        ...
        const configPath = FLOW_CONFIG_FILENAMES.map((filename) => join(directoryPath, filename)).find((candidate) => existsSync(candidate));
        if (!configPath) {
            return [];
        }
...
function summarizeFlowConfig(catalog, directoryName, directoryPath, configPath) {
    try {
...
function matchesFlowQuery(flow, query) {
    return [
        flow.flow_id,
        flow.version,
        flow.description,
