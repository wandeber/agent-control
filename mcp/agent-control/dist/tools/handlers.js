...
export async function handleTool(controller, name, input, signal) {
    ...
    switch (name) {
        ...
        case "agent_definition_configure": {
            ...
            return agentDefinitions.configure(request, {
        ...
        case "agent_definition_delete": {
            const { admin_key, agent_token, ...request } = input;
            return agentDefinitions.delete(request, {
                adminKey: maybeString(admin_key),
                agentToken: maybeString(agent_token)
            });
        }
        ...
        case "run_ack": return controller.acknowledgeRunEvents({ wakeOn: input.wake_on, runId: String(input.run_id), observerAgentId: String(input.observer_agent_id), cursor: String(input.cursor), agentToken: maybeString(input.agent_token), adminKey: maybeString(input.admin_key) });
        ...
        case "flow_catalog_get":
            return controller.getFlowFromCatalog({ flowId: String(input.flow_id), projectDir: input.repo_dir });
        case "flow_start":
        ...
        case "goal_update":
            return controller.updateGoal(String(input.goal_id), input.status);
...
function maybeStringRecord(value) {
    return value && typeof value === "object" && !Array.isArray(value)
...
function stopMode(value) {
    return value === "interrupt" || value === "kill" ? value : "graceful";
}
function assertBlockingWaitAllowed(value) {
    if (value === true || process.env.AGENT_CONTROL_ALLOW_BLOCKING_WAIT === "1") {
        return;
    }
    throw new Error("Blocking waits are disabled by default. Pass allow_blocking_wait=true only for explicit manual/debug waits or short opt-in foreground waits. Normal long-running orchestrators should use detached watchers/subscriptions.");
}
function purgeOptions(input) {
    return {
