...
function stateAt(dir) { return JSON.parse(readFileSync(join(dir, "state.json"), "utf8")); }
function jobs(dir) {
...
function saveJob(dir, entry) { save(join(dir, "queue", `${entry.id}.json`), entry); }
...
const unavailable = (message) => new ControllerError(message, "unsupported_operation");
export function enqueueCliJob(dir, job, initial) {
    mkdirSync(join(dir, "queue"), { recursive: true, mode: 0o700 });
    return locked(dir, () => {
        if (existsSync(join(dir, "cancelled")))
            throw unavailable("CLI worker was cancelled; no new work may be queued.");
...
export function requestCliStop(dir, mode) {
    return locked(dir, () => {
        const state = stateAt(dir);
        const legacy = state.status === "running" && !state.turn_id;
        if (legacy && mode === "interrupt")
            throw unavailable("Recoverable interruption requires the updated CLI supervisor. Wait for this older turn to finish before continuing.");
        if (mode !== "interrupt") {
        ...
        else if (existsSync(join(dir, "cancelled")))
            throw unavailable("CLI worker is durably cancelled; interruption cannot resume it.");
        const target = legacy ? "legacy" : state.status === "running" ? state.turn_id : undefined;
        if (target)
            save(join(dir, "stop.json"), { mode, turn_id: target });
        else if (mode !== "interrupt")
            writeState(dir, { ...state, status: "stopped", updated_at: new Date().toISOString() });
        return target;
    });
}
export async function supervise(dir) {
    const owner = new Database(join(dir, "supervisor-lock.sqlite"));
    try {
        owner.pragma("busy_timeout = 0");
    ...
    try {
        ...
        for (;;) {
            ...
            locked(dir, () => {
                ...
                if (!next || existsSync(join(dir, "cancelled"))) {
                    release();
                    return;
                }
                const state = stateAt(dir);
                ...
                if (next.job.interactive) {
                    ...
                    try {
                        interactive ||= Boolean(new AgentAccessStore(accessDb).getAgentAccess(next.job.interactive.agent_id).requested);
                    }
                    finally {
                        accessDb.close();
                    }
                }
                if (interactive && !next.job.interactive)
                    throw unavailable("The interactive worker binding is unavailable; do not replace its transport.");
                if (interactive)
                    writeState(dir, { ...stateAt(dir), transport: "app-server" });
...
async function runInteractiveTurn(dir, entry, session) {
    ...
    const event = (value) => {
        ...
        if (value.type === "error" || value.type === "turn.failed")
            appendFileSync(join(dir, "stderr.log"), line, { mode: 0o600 });
    };
    let heartbeatAt = 0;
    const heartbeat = () => { if (Date.now() - heartbeatAt >= 1000) {
        heartbeatAt = Date.now();
        locked(dir, () => writeState(dir, { ...stateAt(dir), updated_at: new Date().toISOString() }));
    } };
    ...
    try {
        const result = await runInteractiveCliTurn({ ...entry.job, ...entry.job.interactive }, {
            ...
            stop: () => locked(dir, () => {
                if (existsSync(join(dir, "cancelled")))
                    return readFileSync(join(dir, "cancelled"), "utf8") === "kill" ? "kill" : "graceful";
            ...
            dispatch: send => locked(dir, () => {
                if (existsSync(join(dir, "cancelled")))
                    throw unavailable("The CLI worker was cancelled before dispatch.");
                return send();
            })
        });
        locked(dir, () => {
...
function runTurn(dir, entry, args, session) {
    return new Promise(resolve => {
        ...
        const signal = (mode) => {
            ...
            if (child.pid && signalled !== mode) {
                try {
                    process.kill(-child.pid, mode === "kill" ? "SIGKILL" : mode === "interrupt" ? "SIGINT" : "SIGTERM");
                }
        ...
        child.stdin.end(entry.job.prompt);
        let heartbeatAt = Date.now();
        const timer = setInterval(() => locked(dir, () => {
            if (existsSync(join(dir, "cancelled")))
                signal(readFileSync(join(dir, "cancelled"), "utf8") === "kill" ? "kill" : "graceful");
            else if (existsSync(join(dir, "stop.json"))) {
                const request = JSON.parse(readFileSync(join(dir, "stop.json"), "utf8"));
                if (request.turn_id === entry.id)
                    signal(request.mode);
                rmSync(join(dir, "stop.json"), { force: true });
            }
            if (Date.now() - heartbeatAt >= 1000) {
                heartbeatAt = Date.now();
                writeState(dir, { ...stateAt(dir), updated_at: new Date().toISOString() });
            }
        }), 100);
        let finished = false;
        const finish = (code) => {
