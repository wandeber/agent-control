...
export function readCodexSession(threadId) {
    ...
    const find = (dir) => {
        ...
        for (const entry of readdirSync(dir, { withFileTypes: true })) {
            if (entry.isDirectory()) {
                const found = find(join(dir, entry.name));
                if (found)
                    return found;
            }
    ...
    try {
        if (!path || !existsSync(path))
            path = find(join(home, "sessions")) ?? find(join(home, "archived_sessions"));
        if (!path)
            return null;
        paths.set(key, path);
...
export function sessionUsage(handle) {
    ...
    if (!session?.usage)
        return null;
    const baseline = handle.data.usage_baseline;
    ...
    for (const field of ["input_tokens", "output_tokens", "total_tokens", "cached_input_tokens", "cache_write_input_tokens", "reasoning_output_tokens"]) {
...
export class CodexSessionAdapter {
    ...
    async sendMessageWithReceipt(handle, message) {
        if (handle.data.cli_continuity && !handle.data.prefer_app_server) {
            const id = queueCliMessage(handle.data.cli_continuity, message);
    ...
    async stop(handle) {
        ...
        if (session && writer) {
            ...
            return { status: "stopping", message: "Interrupt sent to the reverified exclusive CLI writer." };
        }
        if (cancelled && session && ["completed", "stopped", "failed"].includes(session.status))
            return { status: "stopped", message: "Pending CLI messages cancelled." };
        await controlExistingSession(this.controlHandle(handle), "interrupt");
        return { status: "stopped", message: "Interrupt accepted by the session's owning app-server." };
    }
    ...
    async getStatus(handle) {
        if (handle.data.remote_session)
            return new CodexThreadAdapter().getStatus(handle);
        const session = readCodexSession(String(handle.data.thread_id));
        ...
        return { status: (queue.uncertain || queue.failed ? "blocked" : queue.pending ? "running" : session?.status ?? "unknown"), updatedAt: session?.updatedAt,
