...
export async function inspectSessionControl(handle) {
    const client = clientFor(handle);
    try {
        await client.initialize();
        const result = await client.request("thread/read", { threadId: String(handle.data.thread_id), includeTurns: true });
        if (result.thread?.id !== String(handle.data.thread_id))
            throw new ControllerError("The control endpoint returned a different thread identity.", "backend_unavailable");
...
export async function controlExistingSession(handle, action, message) {
    if (String(handle.data.app_server_url ?? "").startsWith("stdio"))
        throw new ControllerError("Control requires the session's persistent app-server endpoint; a temporary stdio process cannot own continued work.", "backend_unavailable");
    const client = clientFor(handle);
    try {
        await client.initialize();
        const result = await client.request("thread/read", { threadId: String(handle.data.thread_id), includeTurns: true });
        const thread = result.thread;
        if (thread?.id !== String(handle.data.thread_id))
            throw new ControllerError("The control endpoint returned a different thread identity.", "backend_unavailable");
        ...
        if (thread.status.type === "notLoaded") {
            if (!handle.data.safe_to_resume)
                throw new ControllerError("This endpoint does not own the active session. Connect worker_attach to its owning app-server; no second writer was started.", "backend_unavailable");
            if (action === "interrupt")
                return;
        ...
        if (thread.status.type === "active" && !turn)
            throw new ControllerError("The owning session is active but has no addressable turn yet; retry after its next event.", "backend_unavailable");
        if (action === "interrupt") {
            if (turn)
                await client.request("turn/interrupt", { threadId: String(handle.data.thread_id), turnId: turn.id });
            return;
        }
