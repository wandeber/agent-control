...
export class ConnectionRecovery {
    ...
    now;
    failures = new Map();
    constructor(cooldownMs = 30_000, now = () => Date.now()) {
        this.cooldownMs = cooldownMs;
        this.now = now;
    }
    async connect(key, attempt) {
...
export async function connectWithStartup(connect, start, wait = (ms) => new Promise((resolve) => setTimeout(resolve, ms))) {
    await start();
