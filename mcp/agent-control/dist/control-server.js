...
export async function startControlServer(options) {
            ...
            if (approvalRoute && request.method === "POST") {
                ...
                if (url.pathname === "/api/control/permissions/access-policy-token") {
                    ...
                    if (access.state === "unsupported") {
                        sendJson(response, 409, { error: "This backend does not support access changes." });
                        return;
                    }
                    for (const [key, value] of permissionTokens)
                        if (value.expires < Date.now())
                            permissionTokens.delete(key);
                    const token = randomUUID();
                    permissionTokens.set(token, { agent: agentId, request: `access:${body.revision}`, expires: Date.now() + 60_000 });
                    response.setHeader("Cache-Control", "no-store");
                    sendJson(response, 200, { token });
                    return;
                }
                if (url.pathname === "/api/control/permissions/access-policy") {
                    const token = String(request.headers["x-agent-control-permission"] ?? ""), grant = permissionTokens.get(token);
                    if (!grant || grant.expires < Date.now() || grant.agent !== agentId || grant.request !== `access:${body.revision}`) {
                        sendJson(response, 403, { error: "Missing or expired agent-scoped console capability." });
                        return;
                    }
                    const access = controller.requestAgentAccess(agentId, body.policy, Number(body.revision));
                    permissionTokens.delete(token);
                ...
                if (url.pathname === "/api/control/permissions/access") {
                    ...
                    if (!permission) {
                        sendJson(response, 404, { error: "Permission request not found." });
                        return;
                    }
                    for (const [key, value] of permissionTokens)
                        if (value.expires < Date.now())
                            permissionTokens.delete(key);
                    const token = randomUUID();
                    permissionTokens.set(token, { agent: agentId, request: requestId, expires: Date.now() + 60_000 });
                    response.setHeader("Cache-Control", "no-store");
                    sendJson(response, 200, { token });
                    return;
                }
                if (url.pathname === "/api/control/permissions/decide") {
                    ...
                    if (!grant || grant.expires < Date.now() || grant.agent !== agentId || grant.request !== requestId) {
                        sendJson(response, 403, { error: "Missing or expired request-scoped console capability." });
                        return;
                    }
                    const permission = controller.decidePermission(agentId, requestId, body.decision);
                    permissionTokens.delete(token);
    ...
    wss.on("connection", (client) => {
        ...
        client.on("message", (raw) => {
            try {
                ...
                if (message.type === "select") {
                    ...
                    if (!state.runIds.every(id => typeof id === "string" && id.length > 0))
        ...
        const timer = setInterval(() => {
            ...
            controller.runBackground(async () => {
                try {
                    await tickSocket(controller, client, state);
                }
                finally {
                    ticking = false;
                }
    ...
    return {
        server,
        setUiOrigin: origin => { const value = new URL(origin); if (value.protocol !== "http:" || value.hostname !== "localhost")
            throw new Error("The console must use a localhost origin."); uiOrigin = value.origin; },
...
function sendSelectedSnapshots(controller, client, state) {
...
async function tickSocket(controller, client, state) {
    ...
    if (runIds.length) {
        ...
        await controller.pollActiveAgents();
    if (runIds !== state.runIds || client.readyState !== client.OPEN)
        return;
