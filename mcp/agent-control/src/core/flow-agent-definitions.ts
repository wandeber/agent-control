...
import { AgentDefinitionCatalog, type AgentDefinition } from "./agent-definitions.js";
...
import type { FlowConfig, FlowRoleConfig } from "./types.js";

export interface FlowAgentCatalog {
  resolve(reference: string): { definition: AgentDefinition; revision: string };
}

export function flowAgentRepoDir(role: FlowRoleConfig | undefined, repoDir: string | null): string | null {
...
export function resolveFlowAgentDefinitions(config: FlowConfig, catalog?: FlowAgentCatalog): FlowConfig {
  const resolved = structuredClone(config);
  const selections = new Map<string, ReturnType<FlowAgentCatalog["resolve"]>>();
  for (const [roleId, role] of Object.entries(resolved.roles ?? {})) {
    if (!role.agent_ref) continue;
    if (role.resolved_agent) throw new ControllerError("Flow agent definition was already resolved.", "tool_error", { role: roleId });
    catalog ??= new AgentDefinitionCatalog();
    let selected = selections.get(role.agent_ref);
    if (!selected) {
      try { selected = catalog.resolve(role.agent_ref); }
      catch (cause) { throw new ControllerError(`Cannot resolve agent_ref '${role.agent_ref}' for flow role '${roleId}'.`, "tool_error", { role: roleId, agent_ref: role.agent_ref, cause: cause instanceof Error ? cause.message : String(cause) }); }
...
export function flowSourceConfig(config: FlowConfig): FlowConfig {
  const source = structuredClone(config);
  for (const [id, role] of Object.entries(source.roles ?? {})) {
    if (role.resolved_agent) source.roles![id] = structuredClone(role.resolved_agent.source_role);
  }
  return source;
}

/** Compile only new workers. Callers must retain an existing worker's runtime snapshot. */
export async function compileFlowAgent(role: FlowRoleConfig, repoDir: string) {
