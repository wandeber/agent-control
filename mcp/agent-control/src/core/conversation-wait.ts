import type { RunWakeOn } from "./run-wake-policy.js";

/** Model-facing continuation guidance; launching itself remains non-blocking. */
export function conversationWaitContract(runId: string, observerAgentId: string, wakeOn: RunWakeOn = "control") {
  return {
    ...
    instruction: "While any supervised Agent Control work remains pending, keep this Codex turn open. " +
      ...
      "After an event batch, a timeout, or a user message (including another topic), handle required actions, " +
      ...
      "wait on multiple runs concurrently when supported. Answer user questions in commentary, then resume the pending wait. " +
