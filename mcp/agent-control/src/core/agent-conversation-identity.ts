...
export function agentConversationThreadId(agent: AgentRecord): string | null {
  if (agent.unregistered_at) return null;
  ...
  try {
    ...
    if (realpathSync(resolve(dir)) !== expected) return null;
    if (agent.backend_handle?.access_agent_id !== undefined && agent.backend_handle.access_agent_id !== agent.agent_id) return null;
    ...
    const state = JSON.parse(readFileSync(fd, "utf8")) as Record<string, unknown>;
    if (!state || typeof state !== "object" || Array.isArray(state) ||
        ...
        typeof state.updated_at !== "string" || !Number.isFinite(Date.parse(state.updated_at))) return null;
