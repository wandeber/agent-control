...
export interface PluginInventory extends CapabilityBase {
  id: string;
  name: string;
  description?: string;
  version?: string;
...
export interface AgentDefinitionInventory {
  ...
  inherited_developer_instructions: string;
  ...
  skills: Array<CapabilityBase & { path: string; name: string; description?: string; scope?: string }>;
  mcp_servers: Array<CapabilityBase & { name: string; display_name?: string }>;
  apps: Array<{ id: string; name: string; available: boolean; enabled_by_default: boolean }>;
}

export interface CompiledAgentConfiguration {
  snapshot_version: 1;
  definition_id: string;
  definition_name: string;
  catalog_revision: string;
  inherited_instructions: string;
  definition_instructions: string;
  ...
  plugins: Array<{
    ...
    enabled: boolean;
    required: boolean;
    bundled_skills: Array<{ path: string; name: string; enabled: boolean }>;
  }>;
  skills: Array<{ path: string; enabled: boolean; plugin_id?: string }>;
  mcp_servers: Array<{ name: string; enabled: boolean; plugin_id?: string; root_transport?: boolean }>;
  apps: Array<{ id: string; name: string; enabled: boolean; plugin_ids?: string[] }>;
  app_server_overrides: string[];
  inventory_revision: string;
}
...
const inventoryCache = new Map<string, AgentDefinitionInventory>();

export async function loadAgentDefinitionInventory(
  repoDir: string,
  refresh = false,
  selectedExecutable?: string
): Promise<AgentDefinitionInventory> {
...
export function compileAgentConfiguration(
  ...
  for (const configuredPlugin of plugins) {
    ...
    for (const app of plugin.bundled_apps) {
      ...
      if (!bundle.pluginIds.includes(plugin.id)) bundle.pluginIds.push(plugin.id);
...
export function composeDeveloperInstructions(inherited: string, definition: string): string {
  const parts: string[] = [];
  if (inherited.trim()) parts.push(inherited.trim());
  if (definition.trim()) {
    parts.push(
...
export function compiledSnapshotHash(snapshot: CompiledAgentConfiguration): string {
  return hashJson(snapshot);
}

export function refreshCompiledAgentConfiguration(
  snapshot: CompiledAgentConfiguration,
  inventory: AgentDefinitionInventory
): CompiledAgentConfiguration {
  if (
    inventory.runtime.executable !== snapshot.executable ||
    inventory.runtime.version !== snapshot.executable_version
  ) {
    throw new ControllerError(
      "Configured-agent continuation runtime changed from its immutable snapshot.",
      "unsupported_runtime",
      {
  ...
  const model = inventory.models.find((entry) =>
    entry.id === snapshot.model && entry.model_provider === snapshot.model_provider && entry.available
  );
  if (!model) throw capabilityUnavailable("model", snapshot.model);
  if (model.catalog_available && !model.supported_reasoning_efforts.includes(snapshot.reasoning_effort)) {
    throw capabilityUnavailable(
      "model",
      snapshot.model,
      "The selected reasoning effort is no longer supported by this catalog model."
    );
  }
  ...
  const knownBundledSkillPaths = new Set(snapshot.plugins.flatMap((entry) =>
    entry.bundled_skills.map((skill) => skill.path)
  ));
  ...
  const bundledMcp = inventory.plugins.flatMap((plugin) =>
    plugin.bundled_mcp_servers.map((server) => ({
      name: server.name,
      pluginId: plugin.id,
  ...
  for (const entry of snapshot.mcp_servers) {
    if (entry.enabled && !availableMcpNames.has(entry.name)) throw capabilityUnavailable("mcp_server", entry.name);
  }
  const previousApps = new Map(snapshot.apps.map((entry) => [entry.id, entry.enabled]));
  const currentBundledApps = new Map<string, { name: string; pluginIds: string[] }>();
...
function compileOverrides(input: {
  ...
  plugins: CompiledAgentConfiguration["plugins"];
  skills: CompiledAgentConfiguration["skills"];
  mcpServers: CompiledAgentConfiguration["mcp_servers"];
  apps: CompiledAgentConfiguration["apps"];
}): string[] {
  const bundledMcpByPlugin = new Map<string, Array<{ name: string; enabled: boolean }>>();
  for (const server of input.mcpServers) {
    if (!server.plugin_id) continue;
  ...
  const apps = Object.fromEntries([
    ...
    ...input.apps.map((entry) => [entry.id, { enabled: entry.enabled }] as const)
  ]);
  const result = [
...
async function readRuntime(executable: string, cwd: string, refresh: boolean): Promise<RuntimeRead> {
  return withAppServer(executable, cwd, [], async (client) => {
    ...
    const pluginDetails = new Map<string, JsonObject>();
    ...
    for (const plugin of pluginRows) {
      for (const app of manifestInfo(object(plugin), skills, pluginDetails.get(string(plugin.id))).apps) appIds.add(app.id);
    }
    const appMetadata: JsonObject[] = [];
    const ids = [...appIds];
...
function buildInventory(executable: string, cwd: string, raw: RuntimeRead): AgentDefinitionInventory {
  ...
  const plugins = pluginRows.map((row) => pluginInventory(row, skills, raw.pluginDetails.get(string(row.id))))
    .filter((entry): entry is PluginInventory => Boolean(entry));
  ...
  if (!mcpNames.includes(REQUIRED_AGENT_CONTROL_MCP)) mcpNames.unshift(REQUIRED_AGENT_CONTROL_MCP);

  const configuredModels: Array<{ model: string; provider: string }> = [];
  const addConfiguredModel = (value: JsonObject) => {
  ...
  for (const profile of Object.values(object(config.profiles))) addConfiguredModel(object(profile));
  ...
  for (const plugin of plugins) for (const app of plugin.bundled_apps) appIds.add(app.id);
  return {
    inventory_revision: "",
...
function pluginInventory(row: JsonObject, skills: JsonObject[], detail?: JsonObject): PluginInventory | null {
...
function manifestInfo(row: JsonObject, skills: JsonObject[], detail?: JsonObject): PluginManifestInfo {
  if (detail && Object.keys(detail).length > 0) {
    ...
    const detailSkills = array(detail.skills).map(object).filter((entry) => string(entry.path)).map((entry) => {
      ...
      const runtimeSkill = skills.find((candidate) =>
        string(candidate.name) === name && Boolean(installedRoot) && isInside(string(candidate.path), installedRoot!)
      );
  ...
  if (!root) return { root: null, skills: [], mcpServers: [], apps: [] };
...
async function qualifyRuntime(inventory: AgentDefinitionInventory, cwd: string): Promise<void> {
  ...
  if (!candidate) {
    throw new ControllerError(
      "Codex executable could not be qualified because no enabled plugin skill is available for the isolation probe.",
      "unsupported_runtime",
      { executable: inventory.runtime.executable, version: inventory.runtime.version }
    );
  }
  const disabled = await withAppServer(
    inventory.runtime.executable,
    cwd,
    ["plugins=" + toTomlInline({ [candidate.id]: { enabled: false } })],
    async (client) => {
  ...
  if (!pluginDisabled || !skillsDisabled) {
    ...
    throw new ControllerError(
      "Codex executable is incompatible with configured-agent plugin isolation.",
      "unsupported_runtime",
      {
        executable: inventory.runtime.executable,
...
async function withAppServer<T>(
  ...
  try {
    ...
    throw new ControllerError(
      "Could not read configured-agent inventory from Codex app-server.",
      "unsupported_runtime",
      { executable, error: error instanceof Error ? error.message : String(error) }
    );
  } finally {
...
function resolveCodexExecutable(): string {
  ...
  if (explicit) return resolveExecutable(explicit);
  const bundled = "/Applications/ChatGPT.app/Contents/Resources/codex";
  if (existsSync(bundled)) return realpathSync(bundled);
...
function configuredProfileModels(): JsonObject[] {
  const home = process.env.CODEX_HOME ?? join(homedir(), ".codex");
  try {
    return readdirSync(home)
      .filter((name) => name.endsWith(".config.toml"))
      .map((name) => {
        try { return object(parseToml(readFileSync(join(home, name), "utf8"))); }
        catch { return {}; }
      });
  } catch {
    return [];
...
function pluginRoot(row: JsonObject): string | null {
...
function installedPluginRoot(row: JsonObject): string | null {
  const id = string(row.id);
  ...
  const versionMatches = version ? candidates.filter((root) =>
    string(readJson(join(root, ".codex-plugin", "plugin.json")).version) === version
  ) : [];
...
function capabilityUnavailable(
  kind: string,
  id: string,
  message = "An enabled configured-agent capability is unavailable."
): ControllerError {
...
function hasMcpTransport(value: JsonObject): boolean {
