...
export function parseActivity(value: unknown): AgentActivity | null {
  ...
  if (!text || (item.kind !== "message" && item.kind !== "tool")) return null;
  return { kind: item.kind, text, ...(typeof item.observed_at === "string" ? { observed_at: item.observed_at } : {}),
    ...(["running", "completed", "failed"].includes(String(item.state)) ? { state: item.state as AgentActivity["state"] } : {}) };
}

/** Read the latest public activity in item order, including a tool that has already finished. */
export function codexThreadActivity(turns: Array<{
  status: string;
  startedAt?: number | null;
  completedAt?: number | null;
  items?: Array<{ type: string; text?: string; status?: string; tool?: string; command?: string }>;
}>): AgentActivity | null {
  ...
  for (const item of [...(latest.items ?? [])].reverse()) {
    ...
    if (!tools[item.type]) continue;
    let text = tools[item.type]!;
    // Expose a small known operation vocabulary, never shell arguments, tool inputs, or results.
    if (item.type === "commandExecution") {
      const command = item.command ?? "";
      if (/^\s*(pnpm|npm|yarn)\s+(test|run\s+test)(?:\s|$)/.test(command)) text = "Run tests";
      else if (/^\s*(pnpm|npm|yarn)\s+(build|run\s+build)(?:\s|$)/.test(command)) text = "Build the project";
      else if (/^\s*git\s+diff(?:\s|$)/.test(command)) text = "Review changes";
      else if (/^\s*git\s+status(?:\s|$)/.test(command)) text = "Check changes";
      else if (/^\s*rg(?:\s|$)/.test(command)) text = "Search files";
