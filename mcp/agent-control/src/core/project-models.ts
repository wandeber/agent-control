...
export function resolveProjectRoot(projectDir?: string | null): string | null {
  if (!projectDir) return null;
  if (!isAbsolute(projectDir)) throw new Error("Project directory must be absolute.");
  const start = resolve(projectDir);
  if (!existsSync(start) || !statSync(start).isDirectory()) throw new Error("Project directory does not exist.");
  let current = start;
  let nearest: string | null = null;
  while (true) {
    if (!nearest && existsSync(join(current, ".agents"))) nearest = current;
    if (existsSync(join(current, ".git"))) return current;
    const parent = dirname(current);
    if (parent === current) return nearest ?? start;
    current = parent;
  }
}
...
export function applyProjectModels(config: Record<string, unknown>, projectDir?: string | null): Record<string, unknown> {
  ...
  if (!path || !existsSync(path)) return structuredClone(config);
  const preferences = parse(readFileSync(path, "utf8")) as Record<string, unknown>;
  return applyModelPreferences(config, preferences);
}

export function applyModelPreferences(config: Record<string, unknown>, preferences: Record<string, unknown>): Record<string, unknown> {
  for (const key of Object.keys(preferences)) if (key !== "hdt" && key !== "flows") throw new Error(`Unknown models.toml section: ${key}`);
  for (const [section, groups] of Object.entries(preferences)) {
    if (!record(groups)) throw new Error(`models.toml ${section} must be a table.`);
    const roleGroups = section === "hdt" ? [groups] : Object.values(groups);
    for (const roles of roleGroups) {
      if (!record(roles)) throw new Error("Model overrides must contain role tables.");
  ...
  if (!record(overrides)) return clone;
  const roles = record(clone.roles) ? clone.roles : {};
  for (const [role, override] of Object.entries(overrides)) {
    if (!record(roles[role])) throw new Error(`Unknown role ${role} for flow ${String(config.id)}.`);
