...
export interface PackageDelivery { delivery_id: string; package_id: string; attempt: number; actor_id: string; work_generation: number;
  manifest_digest: string; plan_revision: string; acceptance_revision: number; summary: string; files: PackageFile[]; record_sha256: string; path: string }
export interface PackageBranch { attempt: number; agent_id?: string; state: "pending" | "invoking" | "running" | "delivered" | "accepted" | "failed" | "cancelled" | "uncertain";
  launch_lease_expires_at?: string; work_generation?: number; delivery?: PackageDelivery; acceptance?: { actor_id: string; reason: string }; reason?: string; prior_attempts?: PackageBranch[] }
...
export interface PackageContext { flowId: string; runId: string; repo: string; root: string; planPath: string; planRevision: string; acceptanceRevision: number;
  stepId: string | null; policy: NonNullable<NonNullable<FlowConfig["policy"]>["work_packages"]>; config: FlowConfig; approval?: { value: unknown; acceptance_revision: number; artifact_digest?: string; package_manifest_digest?: string } }
export interface PackageHost {
  ...
  verifyResult(context: PackageContext, id: string): EvidenceReceipt;
...
function regular(root: string, path: string): string {
  ...
  while (cursor !== root) { if (existsSync(cursor) && lstatSync(cursor).isSymbolicLink()) fail("Package files and path ancestors must not be symbolic links."); cursor = dirname(cursor); }
  return absolute;
}
...
export class FlowPackages {
  ...
  private capture(context: PackageContext, group: PackageGroup, entry: WorkPackage, snapshots: boolean): PackageFile[] {
    ...
    for (const path of entry.deliverables) if (!existsSync(regular(entry.worktree, path))) fail(`Required package delivery is missing: ${path}`);
    const files = [...new Set([...changed, ...entry.deliverables])].sort().map(path => {
      ...
      const sha256 = artifactDigest(absolute); const mode = stat.mode & 0o111 ? 0o755 : 0o644;
  ...
  private verifyDelivery(context: PackageContext, group: PackageGroup, entry: WorkPackage, branch: PackageBranch) {
    ...
    if (delivery.actor_id !== branch.agent_id || delivery.attempt !== branch.attempt || delivery.work_generation !== branch.work_generation || delivery.manifest_digest !== group.manifest_digest || delivery.plan_revision !== context.planRevision || delivery.acceptance_revision !== context.acceptanceRevision) fail("Package delivery belongs to a stale owner, generation or plan.");
  ...
  assertJoin(context: PackageContext): PackageGroup {
    ...
    for (const entry of group.manifest) {
      ...
      if (agent && !terminal(agent)) fail(`Package join is waiting for inactive workers: ${entry.id}`);
  ...
  assertIntegrated(context: PackageContext) {
    ...
    const receipt = this.host.verifyResult(context, integration.result_receipt_id);
  ...
  private verifyConsolidation(context: PackageContext, group: PackageGroup) {
    for (const branch of Object.values(group.branches).filter(b => b.state === "accepted")) for (const file of branch.delivery!.files) {
      ...
      if (file.sha256 === null ? existsSync(target) : !existsSync(target) || !lstatSync(target).isFile() || artifactDigest(target) !== file.sha256 || (lstatSync(target).mode & 0o111 ? 0o755 : 0o644) !== file.mode) fail(`Consolidated result does not contain the accepted delivery: ${file.path}`);
    }
  }
  async execute(flowId: string, raw: FlowPackagesRequest): Promise<PackageGroup> {
    ...
    if (request.operation === "define") return this.host.store.immediateTransaction(() => {
      ...
      const manifestDigest = digest({ packages: manifest, plan_revision: context.planRevision, acceptance_revision: context.acceptanceRevision });
    ...
    if (request.operation === "deliver") return this.host.store.immediateTransaction(() => {
      ...
      const agent = this.host.agent(actor); if (agent.unregistered_at || agent.work_generation !== branch.work_generation || ["failed", "stopped"].includes(agent.status) || !["running", "delivered", "accepted"].includes(branch.state)) fail("Package owner or work generation is no longer eligible to deliver.");
      const files = this.capture(context, group, entry, true);
      const body = { delivery_id: `package-${digest({ id: entry.id, attempt: branch.attempt, manifest: group.manifest_digest }).slice(0,32)}`, package_id: entry.id, attempt: branch.attempt, actor_id: actor, work_generation: agent.work_generation, manifest_digest: group.manifest_digest, plan_revision: context.planRevision, acceptance_revision: context.acceptanceRevision, summary: request.summary, files };
    ...
    if (request.operation === "accept") {
      ...
      this.host.store.immediateTransaction(() => {
        ...
        for (const item of request.deliveries) {
          ...
          if (!branch.agent_id || this.host.agent(branch.agent_id).status !== "completed" || this.host.agent(branch.agent_id).unregistered_at || !["delivered", "accepted"].includes(branch.state) || branch.delivery?.delivery_id !== item.delivery_id) fail("Only a delivered, inactive successful branch can be accepted.");
    ...
    if (request.operation === "cancel") {
      const branch = this.group(flowId).branches[request.package_id] ?? fail("Unknown work package.");
      if (request.attempt !== branch.attempt) fail("Cancellation targets an obsolete package attempt.");
      this.host.store.immediateTransaction(() => { const group = this.group(flowId); group.branches[request.package_id]!.state = "cancelled"; group.branches[request.package_id]!.reason = request.reason; delete group.integration; this.save(context, group); });
      if (branch.agent_id && !terminal(this.host.agent(branch.agent_id))) await this.host.stop(branch.agent_id);
      this.host.emit(context, "package_cancelled", { package_id: request.package_id, attempt: request.attempt }); return this.group(flowId);
    }
    if (request.operation === "retry") {
      ...
      if (branch.agent_id) await this.host.refresh(branch.agent_id);
      this.host.store.immediateTransaction(() => {
        const current = this.approved(this.host.context(flowId)); const previous = current.branches[request.package_id]!;
        if (!["failed", "cancelled", "uncertain", "accepted", "delivered"].includes(previous.state) || previous.agent_id && !terminal(this.host.agent(previous.agent_id))) fail("Stop and settle the unsuccessful owner before retrying a package.");
        if (Object.entries(current.branches).some(([id, value]) => id !== request.package_id && value.state !== "pending" && this.entry(current, id).depends_on.includes(request.package_id))) fail("Cancel dependent work before replacing its source delivery.");
        current.branches[request.package_id] = { attempt: previous.attempt + 1, state: "pending", ...(previous.agent_id && ["completed", "failed"].includes(this.host.agent(previous.agent_id).status) && !this.host.agent(previous.agent_id).unregistered_at ? { agent_id: previous.agent_id } : {}), reason: request.reason, prior_attempts: [...(previous.prior_attempts ?? []), { ...previous, prior_attempts: undefined }] }; delete current.integration; this.save(context, current);
      }); return this.launch(this.host.context(flowId));
    }
    return this.host.store.immediateTransaction(() => {
      ...
      const group = this.assertJoin(context); const receipt = this.host.verifyResult(context, request.result_receipt_id);
      this.verifyConsolidation(context, group);
      ...
      const record = digest(body); if (group.integration?.record_sha256 === record) return group;
  ...
  private async launch(context: PackageContext): Promise<PackageGroup> {
    const ready = this.host.store.immediateTransaction(() => {
      ...
      for (const entry of group.manifest) {
        ...
        const agent = branch.agent_id ? this.host.agent(branch.agent_id) : this.host.register(context, entry); branch.agent_id = agent.agent_id; branch.work_generation = agent.work_generation; branch.state = "invoking"; branch.launch_lease_expires_at = new Date(Date.now() + 300_000).toISOString(); changed = true; selected.push({ entry, branch: { ...branch } });
      }
    ...
    const results = await Promise.allSettled(ready.map(async ({ entry, branch }) => {
      try {
        ...
        this.host.store.immediateTransaction(() => {
          ...
          if (current.attempt !== branch.attempt || current.agent_id !== agent.agent_id || current.state !== "invoking") return;
