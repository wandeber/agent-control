...
import type { AgentRecord, EventType } from "./types.js";
...
export type UserQuestionContent = z.infer<typeof userQuestionContentSchema>;
export type UserQuestionAnswers = z.infer<typeof userQuestionAnswersSchema>;
export interface UserQuestionRequest extends UserQuestionContent {
...
type Row = { record_json: string; state: UserQuestionRequest["state"]; answers_json: string | null; answered_at: string | null };
type Emit = (input: { runId: string; agentId: string; type: EventType; payload: Record<string, unknown> }) => unknown;

/** Answers belong to a durable request, never an unsolicited new worker turn. */
export class UserQuestions {
  constructor(private store: SqliteStore, private emit: Emit) {}

  private decode(row: Row): UserQuestionRequest {
  ...
  get(id: string): UserQuestionRequest {
    this.reconcileCancelled();
    const row = this.store.db.prepare("select * from user_questions where question_id=?").get(id) as Row | undefined;
    if (!row) throw new ControllerError("Question not found.", "tool_error");
    return this.decode(row);
  }

  list(runIds: string[], pendingOnly = false): UserQuestionRequest[] {
    this.reconcileCancelled();
    return (this.store.db.prepare(`select * from user_questions where run_id in (select value from json_each(?))
      ${pendingOnly ? "and state='pending'" : ""} order by created_at, rowid`).all(JSON.stringify(runIds)) as Row[]).map(row => this.decode(row));
  }

  ask(agent: AgentRecord, content: UserQuestionContent): UserQuestionRequest {
    ...
    return this.store.immediateTransaction(() => {
      ...
      if (!current || current.unregistered_at || ["stopping", "stopped"].includes(current.status) || !run || ["stopping", "stopped"].includes(run.status)) throw new ControllerError("A stopped execution cannot ask a new question.", "tool_error");
      const previous = this.store.db.prepare("select * from user_questions where agent_id=? and generation=? and request_key=?")
        .get(agent.agent_id, current.work_generation, parsed.request_key) as Row | undefined;
      if (previous) {
        const record = this.decode(previous);
        if (record.title !== parsed.title || JSON.stringify(record.questions) !== JSON.stringify(parsed.questions)) throw new ControllerError("This request_key already identifies a different question.", "tool_error");
        return this.get(record.question_id);
      }
      const record: UserQuestionRequest = { ...parsed, question_id: newId("question"), run_id: agent.run_id, agent_id: agent.agent_id,
  ...
  answer(id: string, input: UserQuestionAnswers): UserQuestionRequest {
    return this.store.immediateTransaction(() => {
      const request = this.get(id);
      const answers = userQuestionAnswersSchema.parse(input);
      if (Object.keys(answers).length !== request.questions.length || Object.keys(answers).some(key => !request.questions.some(item => item.id === key))) throw new ControllerError("Answer every question in this request, without extra fields.", "tool_error");
      const canonical: UserQuestionAnswers = {};
      for (const item of request.questions) {
        const answer = answers[item.id];
        if (!answer || (!answer.text && !answer.option_ids.length) || (!item.multiple && answer.option_ids.length > 1) || new Set(answer.option_ids).size !== answer.option_ids.length || answer.option_ids.some(id => !item.options?.some(option => option.id === id))) throw new ControllerError("Choose a valid option or provide a written answer for each question.", "tool_error");
        canonical[item.id] = { option_ids: [...answer.option_ids].sort(), text: answer.text };
      }
      if (request.state === "answered" && JSON.stringify(request.answers) === JSON.stringify(canonical)) return request;
      if (request.state !== "pending") throw new ControllerError("This question is no longer pending. Refresh to see its recorded answer.", "tool_error");
  ...
  async wait(id: string, timeoutMs = 3_600_000, signal?: AbortSignal): Promise<{ question: UserQuestionRequest; outcome: "answered" | "cancelled" | "timeout"; wait_contract?: { tool: string; arguments: Record<string, unknown> } }> {
    const deadline = Date.now() + z.number().int().positive().max(3_600_000).parse(timeoutMs);
    const first = this.get(id);
    if (first.state !== "pending") return { question: first, outcome: first.state };
    ...
    try {
      while (true) {
        ...
        if (question.state !== "pending") return { question, outcome: question.state };
        if (Date.now() >= deadline) return { question, outcome: "timeout", wait_contract: { tool: "question_wait", arguments: { question_id: id, timeout_ms: 3_600_000 } } };
