...
flowCatalog
  ...
  .action((options: { query?: string }) => output(controller.listFlowCatalog({ query: options.query })));
...
flowCatalog
  ...
  .action((options: { flow: string }) => output(controller.getFlowFromCatalog({ flowId: options.flow })));
