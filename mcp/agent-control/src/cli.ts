#!/usr/bin/env node
import { registerQuestionCommands } from "./cli/questions.js";
...
registerQuestionCommands(program, cliDeps);
...
run
  ...
  .option("--repo-dir <dir>", "Repository directory.")
  .action((options: { title: string; repoDir?: string }) => {
...
run
  ...
  .action((options: { limit?: number }) =>
    output(controller.listRuns(options.limit, { agentToken: authOptions().agentToken }))
  );

run
  .command("get")
  .requiredOption("--run <runId>", "Run id.")
  .action((options: { run: string }) => output(controller.getRun(options.run, { agentToken: authOptions().agentToken })));
...
flowCatalog
  ...
  .action((options: { query?: string; repoDir: string }) => output(controller.listFlowCatalog({ query: options.query, projectDir: options.repoDir })));
...
flowCatalog
  ...
  .option("--repo-dir <path>", "Project directory for project flows and model overrides.", process.cwd())
  .action((options: { flow: string; repoDir: string }) => output(controller.getFlowFromCatalog({ flowId: options.flow, projectDir: options.repoDir })));
...
flow
  ...
  .action(async (options: FlowConfigCliOptions) => {
...
flow
  ...
  .action((options: { flow: string }) => output(controller.getFlowSnapshot(options.flow)));
...
addRequesterOptions(agent
  ...
  .option("--expected-artifact <path>", "Expected artifact path.", collect, [] as string[])
  .option("--attachment <path>", "Attachment path.", collect, [] as string[])
  .action(
    async (options: {
      agent: string;
      prompt?: string;
...
agent
  ...
  .action(
    async (options: {
      ...
      timeout?: string;
      timeoutMs?: number;
      allowBlockingWait?: boolean;
    }) => {
      assertBlockingWaitAllowed(options.allowBlockingWait);
      output(
        await controller.waitForAgentTerminal(options.agent, {
...
agent
  ...
  .action((options: { run?: string; includeUnregistered?: boolean }) =>
    output(controller.listAgents({ runId: options.run, includeUnregistered: options.includeUnregistered }))
  );

agent
  .command("get")
  .requiredOption("--agent <agentId>", "Agent id.")
  .action((options: { agent: string }) => output(controller.getAgent(options.agent)));

agent
  .command("status")
  .requiredOption("--agent <agentId>", "Agent id.")
  .action(async (options: { agent: string }) => output(await controller.refreshAgentStatus(options.agent)));

agent
  .command("send")
  .requiredOption("--agent <agentId>", "Agent id.")
  .requiredOption("--message <message>", "Message text.")
  .action(async (options: { agent: string; message: string }) =>
    output(await controller.sendMessage(options.agent, options.message))
  );
...
agent
  ...
  .action(async (options: { agent: string; limit?: number }) =>
    output(await controller.readLatest(options.agent, options.limit ?? 1))
  );
...
agent
  ...
  .action(async (options: { agent?: string; run?: string; all?: boolean; mode: "graceful" | "interrupt" | "kill" }) => {
    if (options.agent) {
...
agent
  ...
  .action(async (options: { agent?: string; run?: string; all?: boolean }) => {
    if (options.agent) {
...
agent
  ...
  .action(
    async (options: {
      ...
      output(
        await controller.agentPurge(options.agent, {
          dryRun: Boolean(options.dryRun),
          stopFirst: Boolean(options.stopFirst),
          force: Boolean(options.force),
          deleteRuntimeFiles: !options.keepRuntimeFiles
...
subscription
  ...
  .option("--run <runId>", "Run id.")
  .action((options: { subscriber?: string; on: EventType; from?: string; run?: string }) => {
...
subscription
  ...
  .action((options: { run?: string; enabledOnly?: boolean }) => {
    const caller = authOptions().agentToken ? controller.requireAgentToken(authOptions().agentToken) : null;
...
subscription
  ...
  .action((options: { subscription: string }) => output(controller.deleteSubscription(options.subscription)));
...
subscription
  ...
  .action(
    async (options: {
      ...
      assertBlockingWaitAllowed(options.allowBlockingWait);
      output(
        await controller.waitForSubscriptionEvent(options.subscription, {
          intervalMs: options.intervalMs,
          timeoutMs: options.timeoutMs ?? (options.timeout ? parseDurationMs(options.timeout) : undefined)
        })
      );
...
link
  ...
  .option("--run <runId>", "Run id.")
  .option("--agent <agentId>", "Agent id.")
  .action((options: { run?: string; agent?: string }) => {
    const caller = authOptions().agentToken ? controller.requireAgentToken(authOptions().agentToken) : null;
...
link
  ...
  .action((options: { link: string }) => output(controller.deleteAgentLink(options.link)));
...
heartbeat
  ...
  .action((options: { agent: string; idleTimeoutMs: number; reminderIntervalMs?: number }) =>
    output(
      controller.createHeartbeat({
        agentId: options.agent,
...
heartbeat
  .command("list")
  .option("--agent <agentId>", "Agent id.")
  .action((options: { agent?: string }) => output(controller.listHeartbeats(options.agent)));

heartbeat
  .command("delete")
  .requiredOption("--heartbeat <heartbeatId>", "Heartbeat id.")
  .action((options: { heartbeat: string }) => output(controller.deleteHeartbeat(options.heartbeat)));
...
goal
  ...
  .action((options: { agent?: string; objective: string }) => {
    const caller = authOptions().agentToken ? controller.requireAgentToken(authOptions().agentToken) : null;
...
goal
  ...
  .action((options: { goal: string }) => output(controller.getGoal(options.goal)));

goal
  .command("confirm")
  .requiredOption("--goal <goalId>", "Goal id.")
  .option("--no-defer-while-descendants-running", "Ask for confirmation immediately even when child agents are still active.")
  .action(async (options: { goal: string; deferWhileDescendantsRunning?: boolean }) =>
    output(
      await controller.confirmGoal(options.goal, {
...
goal
  ...
  .action(
    async (options: {
      ...
      assertBlockingWaitAllowed(options.allowBlockingWait);
      output(
        await controller.waitForGoalConfirmation(options.goal, {
          intervalMs: options.intervalMs,
          timeoutMs: options.timeoutMs ?? (options.timeout ? parseDurationMs(options.timeout) : undefined)
        })
      );
...
goal
  ...
  .action((options: { goal: string; status: GoalStatus }) =>
    output(controller.updateGoal(options.goal, options.status))
  );

goal
  .command("unregister")
  .requiredOption("--goal <goalId>", "Goal id.")
  .action((options: { goal: string }) => output(controller.deleteGoal(options.goal)));
...
usage
  ...
  .action((options: { run?: string; agent?: string; limit?: number }) =>
    output(controller.listUsageSnapshots({ runId: options.run, agentId: options.agent, limit: options.limit }))
  );
...
program
  ...
  .action((options: { run?: string; agent?: string; type?: EventType; limit?: number }) =>
    output(controller.listEvents({ runId: options.run, agentId: options.agent, type: options.type, limit: options.limit }))
  );
...
program
  ...
  .action((options: { label: string; path: string; run?: string; agent?: string; expected?: boolean }) =>
    output(
...
program
  ...
  .action((options: { path: string; lines?: number }) =>
    output(controller.readArtifactHeader(options.path, options.lines ?? 10))
  );
...
program
  ...
  .option("--run <runId>", "Run id.")
  .action(async (options: { run?: string }) => output(await controller.pollActiveAgents(options.run)));
...
function authOptions(optionsOverride: { allowStoredAdminKey?: boolean } = {}): { agentToken?: string; adminKey?: string } {
  const options = program.opts<{ token?: string; adminKey?: string }>();
...
function flowLaunchNext(start: FlowStartResult, continuation: FlowContinueResult | null): string {
  if (!continuation) {
...
function flowLaunchReason(next: string, continuation: FlowContinueResult | null): string {
  ...
  const reasons: Record<string, string> = {
    ...
    worker_already_running_wait_for_run_events:
      "The active step already has a running worker. Keep this turn open, respond to the user when needed, and resume run_wait with the latest processed cursor.",
    worker_start_in_progress_wait_for_run_events:
      "Another controller owns the durable backend-start lease. Keep this turn open and wait for run events; a timeout or unrelated user message does not end supervision.",
...
function buildFlowUiUrl(input: { host: string; port: number; apiPort: number; runId: string }): string {
...
function compactFlowReportResult(result: FlowStepReportAndContinueResult): Record<string, unknown> {
  return {
...
function assertBlockingWaitAllowed(allowBlockingWait?: boolean): void {
  if (allowBlockingWait || process.env.AGENT_CONTROL_ALLOW_BLOCKING_WAIT === "1") {
    return;
  }
  throw new Error(
    "Blocking waits are disabled by default. Pass --allow-blocking-wait only for explicit manual/debug waits or short opt-in foreground waits. Normal long-running orchestrators should use detached watchers/subscriptions."
  );
}
...
interface FlowConfigCliOptions {
  flow?: string;
  repoDir?: string;
  configFile?: string;
  configJson?: string;
...
async function readConfigOption(options: FlowConfigCliOptions): Promise<Record<string, unknown>> {
  ...
  if (options.flow) return controller.getFlowFromCatalog({ flowId: options.flow, projectDir: options.repoDir ?? process.cwd() }).config;
...
function parseKeyValueList(values: string[]): Record<string, string> | undefined {
  if (values.length === 0) {
    return undefined;
  }
