...
export function registerSmokeCommands(program: Command, deps: CliDeps): void {
  ...
  smoke
    ...
    .action(
      async (options: { repo: string; timeoutMs: number; pollMs: number; keepRun?: boolean; keepWatcherFiles?: boolean }) =>
        deps.output(await smokeWorkerLaunch(options, deps))
    );
}

async function smokeWorkerLaunch(
  options: {
    repo: string;
    timeoutMs: number;
    pollMs: number;
    keepRun?: boolean;
    keepWatcherFiles?: boolean;
  },
  deps: CliDeps
...
function smokePromptPath(): string {
