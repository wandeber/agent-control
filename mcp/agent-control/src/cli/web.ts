...
export function registerWebCommands(program: Command, output: (value: unknown) => void): void {
      ...
      async (options: {
        ...
        if (mode === "static") {
          if (!existsSync(join(webRuntimeDir, "index.html"))) {
            await running.close();
            throw new Error(
              `Agent Control web runtime not found at ${webRuntimeDir}. Run agentctl web build before packaging or use --mode dev for local development.`
            );
          }
...
function resolveWebRuntimeDir(): string {
  return resolve(dirname(fileURLToPath(import.meta.url)), "../../../../web-runtime");
}

async function buildWebRuntime(webDir: string): Promise<void> {
  if (!existsSync(join(webDir, "package.json"))) {
    throw new Error(`Agent Control web package not found: ${webDir}`);
  }
...
function resolveStaticFile(root: string, rawPathname: string): string | null {
...
function sendStaticFile(response: ServerResponse, filePath: string, headOnly: boolean): void {
...
function sendStaticError(response: ServerResponse, status: number, message: string): void {
