...
import { ConsoleSessions, type FlowConsoleSelection } from "./console-sessions.js";
...
const REUSE_CONSOLE_TOOL = {
  ...
  description:
    "Reuse and update the already-open Agent Control panel without opening another tab. Set screen: flows, flow_id and repo_dir to live-preview source definitions while authoring, without starting work. Optionally pin run_id; use this after the first open_agent_control_console call.",
  inputSchema: {
    type: "object",
    properties: {
      ...FLOW_SELECTION_PROPERTIES,
...
const CLOSE_CONSOLE_TOOL = {
  ...
  description:
    "Request the currently-open Agent Control panel to close. The MCP host may decline the teardown; this command never opens a new panel.",
  inputSchema: {
    type: "object",
    properties: {},
    additionalProperties: false
...
> {
  ...
  if (name === "agent_control_console_access_request") {
    ...
    return { content: [{ type: "text", text: "Saved requested access for the next turn." }], structuredContent: { access: controller.requestAgentAccess(agentId, (input as Record<string, unknown>).policy as AccessPolicy, numberField(input, "revision", -1)) } };
  }
  if (name === "agent_control_console_permission_decide") {
    const agentId = requiredStringField(input, "agent_id");
    assertConsoleAgent(input, agentId);
    const decision = requiredStringField(input, "decision");
    if (decision !== "approve" && decision !== "reject") throw new Error("Invalid permission decision.");
