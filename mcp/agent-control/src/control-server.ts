    ...
    try {
      ...
      if (approvalRoute && request.method === "POST") {
        ...
        if (url.pathname === "/api/control/questions/access" || url.pathname === "/api/control/questions/answer") {
          ...
          if (!question) { sendJson(response, 404, { error: "Question not found for this agent." }); return; }
          ...
          if (url.pathname.endsWith("/access")) {
            ...
            const token = randomUUID(); permissionTokens.set(token, { agent: input.agent_id, request: requestKey, expires: Date.now() + 60_000 });
          ...
          if (!grant || grant.expires < Date.now() || grant.agent !== input.agent_id || grant.request !== requestKey) { sendJson(response, 403, { error: "Missing or expired question-scoped console capability." }); return; }
          const answered = controller.answerConsoleQuestion(input.agent_id, input.question_id, input.answers);
          permissionTokens.delete(token); sendJson(response, 200, { question: answered }); return;
        }
  ...
  if (request.method === "POST" && url.pathname === "/api/control/links") {
    ...
    sendJson(
      ...
      controller.createAgentLink({
        ...
        type: stringField(body, "type") as AgentLinkType,
...
function sendSocket(client: WebSocket, payload: unknown): void {
  if (client.readyState === client.OPEN) {
...
function optionalObjectField(body: Record<string, unknown>, key: string): Record<string, unknown> | null {
  const value = body[key];
  return value && typeof value === "object" && !Array.isArray(value) ? (value as Record<string, unknown>) : null;
}

function missingStringField(key: string): never {
  throw new Error(`Missing string field: ${key}`);
}
