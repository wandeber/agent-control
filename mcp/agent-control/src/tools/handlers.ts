...
export async function handleTool(
  ...
  switch (name) {
    ...
    case "canvas_positions_set": {
      ...
      return name === "canvas_positions_get" ? controller.getCanvasPositions(String(input.run_id)) : controller.setCanvasPositions(String(input.run_id), Number(input.expected_revision), input.positions as import("../core/canvas-positions.js").CanvasPosition[]);
    }
    case "flow_packages": return controller.executeFlowPackages({ flowInstanceId: String(input.flow_instance_id), request: input.request as import("../core/flow-packages.js").FlowPackagesToolRequest, agentToken: maybeString(input.agent_token), adminKey: maybeString(input.admin_key), signal });
