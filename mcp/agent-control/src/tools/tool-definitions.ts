...
import { AGENT_DEFINITION_TOOL_DEFINITIONS } from "./agent-definition-tools.js";
import { workerAttachSchema } from "./schemas.js";
import {
  flowPackagesSchema, flowContextUpdateSchema, flowOwnerRecoverSchema, flowDecisionSchema, flowEvidenceSchema, runAckSchema,
  workerLaunchSchema,
  flowLaunchSchema,
  agentLinkCreateSchema,
  agentLinkDeleteSchema,
  agentLinkListSchema,
  agentIdSchema,
  agentListSchema,
  agentReadLatestSchema,
  agentRegisterSchema,
  agentPurgeSchema,
  agentSendSchema,
  agentStartSchema,
  agentStopSchema,
  agentWaitSchema,
  agentExternalSyncSchema,
  artifactReadHeaderSchema,
  artifactRegisterSchema,
  emptySchema,
  eventListSchema,
  flowCatalogGetSchema,
  flowCatalogListSchema,
  flowContinueSchema,
  flowDispatchActiveSchema,
  flowGetSchema,
  flowStartSchema,
  flowStepReportSchema,
  flowStepStartSchema,
  flowValidateConfigSchema,
  goalGetSchema,
  goalRegisterSchema,
  goalUpdateSchema,
  goalWaitConfirmationSchema,
  heartbeatCreateSchema,
  heartbeatDeleteSchema,
  heartbeatListSchema,
  maintenancePurgeOldSchema,
...
export const TOOL_DEFINITIONS: ToolDefinition[] = [
  ...QUESTION_TOOLS,
  ...AGENT_DEFINITION_TOOL_DEFINITIONS,
  ...
  {
    name: "run_wait",
    description: "Keep this tool call pending while work runs; a subscription alone does not wait. Default control wakes for decisions owned by this thread, intervention and aggregate completion; an executing coordinator also receives routing events. Use all only for explicitly requested individual events. Acknowledge handled deliveries with run_ack; omit cursor to resume durable processed progress. For conversational supervision, use timeout_ms: 3600000 (one hour) on every call and reentry. A lower value requires an explicit user timeout or discretion to choose. Do not lengthen it without a user instruction or a concrete reason within granted discretion. This minimum is the configured timeout, not elapsed waiting: matching actionable events, failures, blockers and completion return early. Outer async yields or a 60-second responsiveness guideline do not shorten this timeout; resume the existing pending wrapper call without duplicating run_wait. Report a host deadline below one hour instead of silently shortening the timeout without user authorization. Tool cancellation ends only this wait and never acknowledges events. After handling an error or answering new user input, resume the same run/observer with the durable processed cursor and the accepted timeout policy unless the user explicitly pauses or cancels supervision. If the original call is still pending, resume it instead. Preserve all active runs. A timeout or unrelated topic is not completion; keep the turn open and reattach. Each response provides the next wait_contract. completion reports the outcome only when all registered work and descendants settle. A single worker/flow completion is not whole-run completion; closed only closes that observation. Do not relay routine progress or create monitoring automations.",
    inputSchema: objectSchema({ run_id: stringProperty("Observed run."), observer_agent_id: stringProperty("Observing participant id."),
      ...
      cursor: stringProperty("Explicit replay cursor; omit to resume durable acknowledged progress."), timeout_ms: numberProperty("Conversational default/minimum: 3600000 ms (one hour); do not lengthen without a user instruction or a concrete reason within granted discretion. A lower value requires an explicit user timeout or discretion to choose; preserve that policy on reentry. Outer wrapper yields do not lower it. Short explicit diagnostics/tests and indefinite CLI/internal omission remain supported."), limit: numberProperty("Maximum batch size, up to 100.") }, ["run_id", "observer_agent_id"]),
  ...
  },
  {
    ...
    description:
      "List bundled and user flow catalogs plus .agents/flows from explicit repo_dir. Project flow IDs replace matching installed IDs.",
    inputSchema: objectSchema({
  ...
  },
  {
    ...
    description:
      "Resolve a flow by ID or directory name, with project-local shadowing and deterministic .agents/models.toml overrides when repo_dir is supplied.",
  ...
  },
  {
    name: "flow_start",
    description:
      "Create a flow instance, bind it to a run, and activate the initial step. Native codex-subagent flows return one scoped bridge credential at creation time.",
    inputSchema: objectSchema(
      {
        ...requesterProperties,
        config: { type: "object", description: "Flow config object." },
  ...
  },
  {
    ...
    description:
      "Dispatch the active flow step to its configured backend worker, optionally subscribe an orchestrator to terminal events, and return immediately. The caller keeps its turn open and resumes run_wait after user replies or event processing while work remains.",
    inputSchema: objectSchema(
      {
  ...
  },
  {
    name: "flow_continue",
    description:
      "Advance a flow deterministically: dispatch the active unassigned step, wait when a step is already running, or stop at configured notifications and blockers.",
    inputSchema: objectSchema(
      {
        flow_instance_id: stringProperty("Flow instance id."),
  ...
  },
  {
    ...
    description:
      "Claim one scoped native subagent action for 60 seconds. A successful claim returns the private request and one-time action token; claims during the lease return already_claimed without either secret.",
    inputSchema: objectSchema(
      {
        action_id: {
  ...
  },
  {
    ...
    description:
      "Acknowledge a claimed native action with its rotated action token. Identical acknowledgements are idempotent; conflicting acknowledgements are rejected.",
    inputSchema: objectSchema(
      {
        action_id: {
  ...
  },
  {
    ...
    description:
      "Synchronize one codex-subagent card from root-owned subagent v2 state without semantically completing its flow step.",
    inputSchema: objectSchema(
      {
        ...
        native_status: enumProperty(
          ["pending_init", "running", "completed", "interrupted", "shutdown", "errored", "missing"],
  ...
  },
  {
    ...
    description:
      "Report the result of an active flow step. The controller validates result schema, required artifacts, selects the next transition, and auto-continues by default.",
    inputSchema: objectSchema(
      {
  ...
  },
  {
    ...
    description:
      "Manually activate a configured flow step, typically after a notify/orchestrator decision. Required inputs are validated before the step becomes active.",
    inputSchema: objectSchema(
      {
        flow_instance_id: stringProperty("Flow instance id."),
        ...
        reason: stringProperty(
          "Optional coordinator context and transition reason delivered to the manually activated step."
        )
      },
  ...
  },
  {
    ...
    inputSchema: objectSchema(
      { run_id: stringProperty("Run id."), agent_token: stringProperty("Optional agent identity token.") },
      ["run_id"]
    ),
    schema: runIdSchema
  },
  {
    name: "run_shutdown",
    description:
      "Stop active agents in a run. Returns stopped agents plus scoped native actions; the run remains stopping until every native worker is terminal.",
  ...
  },
  {
    name: "agent_wait",
    description:
      "Block until one agent reaches a terminal status, refreshing status cheaply. Use for explicit manual/debug waits or short opt-in foreground waits. Normal long-running orchestrators should use detached watchers/subscriptions.",
    inputSchema: objectSchema(
      {
        agent_id: stringProperty("Agent id."),
  ...
  },
  {
    ...
    description:
      "Block until a subscription's matching event exists, refreshing the source agent when possible. Use this when the current coordinator must stay in its turn and receive the event directly; set allow_blocking_wait=true. The result includes matched and delivered, so an event is still returned when the subscriber backend cannot receive an inbound message. Normal long-running orchestrators may use detached watchers instead.",
    inputSchema: objectSchema(
      {
  ...
  },
  {
    name: "goal_confirm",
    description:
      "Ask the goal owner whether the goal is complete, should continue, or is blocked. Defers by default while child agents are active.",
    inputSchema: objectSchema(
      {
        goal_id: stringProperty("Goal id."),
        defer_while_descendants_running: booleanProperty(
          "Defer confirmation while parent_child descendants are active."
        )
      },
      ["goal_id"]
    ),
  ...
  },
  {
    ...
    description:
      "Wait in deterministic code until the goal owner has no active child descendants, then ask for goal completion confirmation. Use for explicit manual/debug waits or short opt-in foreground waits. Normal long-running orchestrators should use detached watchers/subscriptions.",
    inputSchema: objectSchema(
      {
        goal_id: stringProperty("Goal id."),
        interval_ms: numberProperty("Refresh interval in milliseconds."),
