import { resolveCliContinuity, type CliContinuity } from "../adapters/attached-cli-bridge.js";
...
export async function attachWorkerTool(controller: AgentController, input: Record<string, unknown>) {
  ...
  const endpoint = input.server as string | undefined ?? previous?.backend_handle?.app_server_url as string | undefined ?? process.env.CODEX_APP_SERVER_URL ?? process.env.CODEX_APP_SERVER;
  ...
  let controlled: Awaited<ReturnType<typeof inspectSessionControl>> | undefined;
  let controlError: string | undefined;
  try { controlled = await inspectSessionControl(controlHandle); }
  catch (error) { controlError = error instanceof Error ? error.message : "Control endpoint unavailable."; }
  if (!session && !controlled) throw new ControllerError("The existing Codex session is not readable locally or through its owning app-server. Supply server for the owning host; do not relaunch it.", "backend_unavailable");
  ...
  let continuity: CliContinuity | undefined;
  let continuityError: string | undefined;
  if (session) {
    try { continuity = resolveCliContinuity(threadId, input.profile as string | undefined ?? previous?.backend_handle?.profile as string | undefined, writer); }
    catch (error) { continuityError = error instanceof Error ? error.message : "Original CLI configuration is unavailable."; }
  }
  ...
  if (agent.backend === "codex-session") {
    if (reused) agent = controller.reconnectAttachedSession(agent.agent_id, {
