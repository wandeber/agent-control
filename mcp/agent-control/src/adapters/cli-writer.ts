...
export interface CliWriter { pid: number; started: string; executable: string; profile?: string; configurationOverrides: boolean; unsupportedOverrides?: boolean }
const run = (bin: string, args: string[]) => execFileSync(bin, args, { encoding: "utf8", timeout: 4000, stdio: ["ignore", "pipe", "ignore"] }).trim();

export function hasRolloutWriter(path: string): boolean {
  try { return run("lsof", ["-nP", "-Fa", "--", path]).split("\n").some(row => /^a[wu]$/.test(row)); }
...
export function discoverCliWriter(path: string): CliWriter | null {
  if (process.platform === "win32") return null;
  try { path = realpathSync(path); } catch { return null; }
  let rows: string[];
  try { rows = run("lsof", ["-nP", "-Fpcfan", "--", path]).split("\n"); } catch { return null; }
  const candidates = new Set<number>();
  let pid = 0, access = "";
  for (const row of rows) {
    if (row[0] === "p") pid = Number(row.slice(1));
    if (row[0] === "f") access = "";
    if (row[0] === "a") access = row.slice(1);
    if (row === `n${path}` && /[wu]/.test(access)) candidates.add(pid);
  }
  ...
  try {
    ...
    if (!/^\S*codex\s+exec(?:\s|$)/.test(command)) return null;
    const files = run("lsof", ["-nP", "-a", "-p", String(pid), "-Ffan"]).split("\n");
    const rollouts = new Set<string>();
    let fd = "", mode = "", executable = "";
    for (const row of files) {
      if (row[0] === "f") { fd = row.slice(1); mode = ""; }
      if (row[0] === "a") mode = row.slice(1);
      if (row[0] === "n") {
        const name = row.slice(1);
        if (fd === "txt" && basename(name) === "codex") executable = name;
        if (/[wu]/.test(mode) && /rollout-.*\.jsonl$/.test(name)) rollouts.add(name);
      }
    }
    if (!executable || rollouts.size !== 1 || !rollouts.has(path)) return null;
    const profile = command.match(/(?:^|\s)(?:--profile|-p)(?:=|\s+)([A-Za-z0-9][A-Za-z0-9_.-]*)(?=\s|$)/)?.[1];
    if (run("ps", ["-p", String(pid), "-o", "lstart="]) !== started) return null;
...
export function interruptCliWriter(path: string, expected: CliWriter): void {
