...
import { cancelQueuedCliMessages, cliQueueState, ensureCliBridge, queueCliMessage, type CliContinuity, type DispatchFence } from "./attached-cli-bridge.js";
...
const usageFields = ["input_tokens", "output_tokens", "total_tokens", "cached_input_tokens", "cache_write_input_tokens", "reasoning_output_tokens"] as const;
type SessionBaseline = AgentUsageObservation & { rollout_path?: string; rollout_usage_index?: number };

function belongsToThread(path: string, home: string, threadId: string): boolean {
  let fd: number | undefined;
  try {
    const actual = realpathSync(path);
    if (!actual.endsWith(".jsonl") || !["sessions", "archived_sessions"].some(dir => {
      if (!existsSync(join(home, dir))) return false;
...
function sessionPath(home: string, threadId: string): string | undefined {
  ...
  for (const file of readdirSync(home).filter(name => /^state_\d+\.sqlite$/.test(name)).sort((a, b) => Number(b.slice(6, -7)) - Number(a.slice(6, -7)))) {
    ...
    try {
      ...
      if (row?.rollout_path && belongsToThread(row.rollout_path, home, threadId)) return row.rollout_path;
...
function sessionCandidates(home: string, threadId: string): string[] {
  ...
  const visit = (dir: string) => {
    ...
    for (const entry of readdirSync(dir, { withFileTypes: true })) {
      ...
      else if (entry.isFile() && suffix.test(entry.name)) candidates.push(path);
    }
  };
...
const usagePaths = new Map<string, { active?: string; paths: string[] }>();

/** Historical usage may span resumed rollouts; every path must still prove its native thread identity. */
export function codexUsagePaths(threadId: string, baselinePath?: string): string[] {
  if (!/^[a-f0-9]{8}(?:-[a-f0-9]{4}){3}-[a-f0-9]{12}$/i.test(threadId)) return [];
  ...
  try {
    ...
    if (usagePaths.size >= 100 && !usagePaths.has(key)) usagePaths.delete(usagePaths.keys().next().value!);
    usagePaths.set(key, { active, paths });
    return [...new Set([baselinePath, active, ...paths].filter((path): path is string => !!path && belongsToThread(path, home, threadId)).map(path => realpathSync(path)))];
  } catch { return []; }
}
...
export function sessionUsage(handle: AgentHandle): AgentUsageObservation | null {
  ...
  let baseline = handle.data.usage_baseline as SessionBaseline | undefined;
  ...
  const exactBoundary = baseline?.rollout_path === session.path && typeof index === "number" && (
    index === -1 && baseline.source === "codex.rollout.attachment_baseline" && usageFields.every(field => baseline![field] === 0) ||
...
export function sessionBaseline(threadId: string): AgentUsageObservation | null {
  ...
  return { ...baseline, rollout_path: session.path, rollout_usage_index: session.usageSamples.length - 1 } as SessionBaseline;
}
...
export class CodexSessionAdapter implements AgentAdapter {
  ...
  async interrupt(handle: AgentHandle) {
    const session = handle.data.remote_session ? null : readCodexSession(String(handle.data.thread_id));
    const writer = session ? discoverCliWriter(session.path) : null;
    if (session && writer) {
      interruptCliWriter(session.path, writer);
      return { status: "running" as const, message: "Interrupt sent to the reverified exclusive CLI writer; awaiting its turn result." };
    }
    if (!handle.data.prefer_app_server && session && ["completed", "stopped", "failed"].includes(session.status)) return this.getStatus(handle);
    await controlExistingSession(this.controlHandle(handle), "interrupt");
    return { status: "running" as const, message: "Interrupt accepted by the session's owning app-server; awaiting its turn result." };
  }
  async stop(handle: AgentHandle, options?: StopOptions) {
    if (options?.mode === "interrupt") return this.interrupt(handle);
    cancelQueuedCliMessages(String(handle.data.thread_id));
    const result = await this.interrupt(handle);
    return { ...result, status: ["completed", "stopped", "failed", "waiting_for_input"].includes(result.status) ? "stopped" as const : "stopping" as const };
  }
  ...
  private observedStatus(status: AgentStatus, handle: AgentHandle): AgentStatus {
    if (handle.data.cancel_requested && ["stopped", "completed", "failed"].includes(status)) return "stopped";
