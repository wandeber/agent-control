...
import type { AgentUsageObservation } from "../core/types.js";

const fields = ["input_tokens", "output_tokens", "total_tokens", "cached_input_tokens", "cache_write_input_tokens", "reasoning_output_tokens"] as const;
type Field = typeof fields[number];
type Counts = Record<Field, number | null>;
interface Sample { counts: Counts; at: string; model: string | null; path: string; ordinal: number; createdAt: string; fresh: boolean; contextLimit: number | null }
const cache = new Map<string, { signature: string; samples: Sample[] }>();
const count = (n: unknown): number | null => typeof n === "number" && Number.isSafeInteger(n) && n >= 0 ? n : null;

function samplesFrom(path: string): Sample[] {
  const stat = statSync(path), signature = `${stat.size}:${stat.mtimeMs}`;
  if (cache.get(path)?.signature === signature) return cache.get(path)!.samples;
  let model: string | null = null, createdAt = "";
  ...
  for (const line of readFileSync(path, "utf8").split("\n")) {
    ...
    const p = row.payload;
    if (row.type === "session_meta") createdAt = row.timestamp;
    if (row.type === "turn_context" && typeof p?.model === "string") model = p.model;
    if (row.type !== "event_msg" || p?.type !== "token_count" || !p.info?.total_token_usage || !Number.isFinite(Date.parse(row.timestamp))) continue;
    const usage = p.info.total_token_usage, last = p.info.last_token_usage;
    const counts = Object.fromEntries(fields.map(field => [field, count(usage[field])])) as Counts;
...
export function codexRunUsage(threadId: string, start: string, end: string | null, baselinePath?: string): AgentUsageObservation | null {
  return codexRunUsageWithEvidence(threadId, start, end, baselinePath)?.usage ?? null;
}

export interface UsageEvidence { at: number; key: string }
const fingerprint = (sample: Sample) => JSON.stringify([Date.parse(sample.at), sample.model, sample.counts]);

/** Private record fingerprints let persistence detect missing segments, even in a truncated file. */
export function codexRunUsageWithEvidence(threadId: string, start: string, end: string | null, baselinePath?: string): { usage: AgentUsageObservation; evidence: UsageEvidence[] } | null {
  const from = Date.parse(start), until = end === null ? Infinity : Date.parse(end);
  if (!Number.isFinite(from) || Number.isNaN(until) || until < from) return null;
  ...
  for (const path of codexUsagePaths(threadId, baselinePath)) {
    for (const sample of samplesFrom(path)) {
      if (Date.parse(sample.at) > until) continue;
      // Resumed files can contain copied history. Identical native records count once.
      const key = fingerprint(sample);
      const previous = unique.get(key);
      if (!previous || Date.parse(sample.createdAt) < Date.parse(previous.createdAt)) unique.set(key, sample);
    }
  }
  const samples = [...unique.values()].sort((a, b) => Date.parse(a.at) - Date.parse(b.at) || a.ordinal - b.ordinal);
  const totals = Object.fromEntries(fields.map(field => [field, 0])) as Counts;
  const models = new Set<string | null>();
  const evidence: UsageEvidence[] = [];
  const remember = (sample: Sample) => evidence.push({ at: Date.parse(sample.at), key: createHash("sha256").update(fingerprint(sample)).digest("hex") });
  let previous: Sample | undefined, latest: Sample | undefined;
  for (const sample of samples) {
    // The counter at the start is the baseline: adjacent windows are (start, end].
    if (Date.parse(sample.at) <= from) { previous = sample; continue; }
    if (!latest && previous) remember(previous);
    remember(sample);
    ...
    if (changed) models.add(sample.model);
    latest = sample; previous = sample;
  }
