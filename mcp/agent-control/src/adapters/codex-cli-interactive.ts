...
export interface InteractiveTurnControl {
  ...
  turn(id: string): void;
  heartbeat(): void;
  stop(): "interrupt" | "graceful" | "kill" | undefined;
  /** Recheck cancellation and send the already-connected request under the dispatch lock. */
...
export async function runInteractiveCliTurn(job: InteractiveCliJob, control: InteractiveTurnControl): Promise<{ thread_id?: string; status: "completed" | "failed" | "waiting_for_input" }> {
  ...
  let settle!: (result: { thread_id?: string; status: "completed" | "failed" | "waiting_for_input" }) => void;
  const done = new Promise<{ thread_id?: string; status: "completed" | "failed" | "waiting_for_input" }>(resolve => { settle = resolve; });
  const items = new Map<string, Record<string, any>>();
  const early: Array<{ method: string; params?: unknown }> = [];
  const finish = (status: "completed" | "failed" | "waiting_for_input") => {
    if (turnFinished) return;
    turnFinished = true;
    settle({ thread_id: threadId, status });
  };
  const confirmAccess = (response: Record<string, any>) => {
    if (!threadId) return;
  ...
  const notification = (message: { method: string; params?: unknown }) => {
    ...
    if (message.method === "thread/settings/updated") { confirmAccess(record(p.threadSettings)); return; }
    ...
    if (message.method === "item/started" || message.method === "item/completed") {
      ...
      if (turn.status === "completed") { control.event({ type: "turn.completed", turn_id: turnId }); finish("completed"); }
      else if (turn.status === "interrupted") finish("waiting_for_input");
      else { control.event({ type: "turn.failed", error: turn.error, turn_id: turnId }); finish("failed"); }
    }
  };
  ...
  try {
    ...
    if (typeof actualId !== "string" || (control.session && actualId !== control.session)) throw new Error("Codex interactive continuation returned a different session.");
    threadId = actualId;
    ...
    for (const message of early.splice(0)) notification(message);
    client.bindApprovalTurn?.(turnId);
    if (record(started.turn).status === "completed") finish("completed");
    ...
    try {
      ...
      if (record(effective.thread).id === threadId) confirmAccess(effective);
    } catch { /* Keep an unconfirmed access revision pending without inventing an applied policy. */ }
    let checking = false;
    timer = setInterval(() => {
      if (checking || turnFinished) return;
      checking = true;
      try {
        control.heartbeat();
        const mode = control.stop();
        if (mode === "kill") { closing = true; client.close(true); finish("failed"); }
    ...
    removeNotification(); removeClose();
    try { await client.closeAndWait(); } finally { db.close(); }
  }
}

function cliItem(item: Record<string, any>): Record<string, any> {
  const types: Record<string, string> = { agentMessage: "agent_message", commandExecution: "command_execution", mcpToolCall: "mcp_tool_call", webSearch: "web_search", fileChange: "file_change", collabAgentToolCall: "collab_tool_call" };
...
async function startup<T>(operation: Promise<T>): Promise<T> {
  let timer: ReturnType<typeof setTimeout> | undefined;
  try { return await Promise.race([operation, new Promise<never>((_, reject) => { timer = setTimeout(() => reject(new Error("Codex interactive setup did not respond; the same execution is retained.")), 15000); })]); }
