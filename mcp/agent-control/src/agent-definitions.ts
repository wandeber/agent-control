...
import {
  ...
  configureAgentDefinitionSchema,
  deleteAgentDefinitionSchema,
  type ConfigureAgentDefinitionInput,
  type DeleteAgentDefinitionInput
...
import { EVENT_TYPES, type AgentRecord } from "./core/types.js";
...
const requesterFields = {
  ...
  requester_delivery: z.enum(["wait", "notify"] as const).optional()
};
...
export const agentDefinitionLaunchSchema = z.object({
  ...
  sandbox: z.enum(["read_only", "workspace"] as const).optional(),
...
export type AgentDefinitionLaunchInput = z.infer<typeof agentDefinitionLaunchSchema>;

export class AgentDefinitionService {
  constructor(
    readonly controller: AgentController,
    readonly catalog = new AgentDefinitionCatalog()
  ) {}
  ...
  get(input: z.infer<typeof agentDefinitionGetSchema>) {
  ...
  configure(
    input: ConfigureAgentDefinitionInput,
    authority: CatalogMutationAuthority = {}
  ) {
  ...
  async launch(input: unknown, deps: CliDeps) {
    ...
    const configuredAgent = compileAgentConfiguration(
      selected.definition,
      selected.revision,
      inventory,
      repoDir
    ...
    if (!snapshotHash) {
      throw new ControllerError(
        "Configured worker started without a verified immutable snapshot.",
        "tool_error",
        { definition_id: selected.definition.definition_id, agent_id: executionAgentId }
      );
    }
    return {
      ...launch,
...
export interface CatalogMutationAuthority {
  adminKey?: string;
  agentToken?: string;
  currentThreadId?: string;
  trustedCatalogCapability?: boolean;
}

export function assertCatalogMutationAuthority(
  controller: AgentController,
  authority: CatalogMutationAuthority
): void {
  if (authority.agentToken) {
    throw new ControllerError(
      "Worker and run-scoped credentials cannot modify the personal configured-agent catalog.",
      "auth_required"
    );
  }
  ...
  if (threadId && managedAgentsForThread(controller, threadId).length > 0) {
    throw new ControllerError(
      "A managed worker thread cannot modify the personal configured-agent catalog.",
      "auth_required"
    );
  }
  ...
  if (!verifyAdminKey(adminKey)) {
    throw new ControllerError(
      "Configured-agent catalog changes require local operator authorization.",
      "auth_required"
    );
  }
}

export function managedAgentsForThread(
  controller: AgentController,
  threadId: string
): AgentRecord[] {
  return controller.listAgents().filter((agent) => {
    if (agent.unregistered_at) return false;
